...
Backbone & Four-scale GroupNorm encoder-decoder, base width 128 and
condition dimension 160. \\
Residual distribution & Three mixture components per task/region head. \\
Residual limits & Path loss: 7 dB LoS and 25 dB NLoS; delay spread: 20 ns LoS
and 20 ns NLoS; angular spread: 17 degrees LoS and 15 degrees NLoS. \\
Main losses & Final fine-tune weights: RMSE 1.0, MAE 0.05, anchor 0.01,
prior-guard 0.02; NLL, KL, moment matching, and outlier budget are present in
the implementation but have zero weight in the final resolved configuration. \\
Training & AdamW, learning rate \(5\times10^{-5}\), weight decay 0.01,
40 fine-tune epochs, batch size 1, gradient accumulation 8, cosine
\texttt{LambdaLR} after one warmup epoch. \\
\bottomrule
\end{tabularx}
\end{table}
...
the final huge path-loss-finetuned checkpoint, DirectML inference, and
