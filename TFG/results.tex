...
inspection panel. The same plotting script is used for the appendix panels:
the first three rows compare ground truth, frozen prior, and model output; the
fourth row shows topology and visibility masks; the last two rows show the
model error and the residual change applied to the prior.
...
\caption{Representative Try 80 inspection panel for a validation sample. The
panel shows the three targets, their frozen priors, the model outputs,
topology and LoS/NLoS support, model error, and model-prior residual maps.}
