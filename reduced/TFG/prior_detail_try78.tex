...
where $\rho \in [0, 1.5]$ is the effective amplitude (allowing over unity
values during the calibration search) and $\phi \in [-\pi,\pi]$ is the
effective phase offset. This range is a robustness allowance for an effective
CKM fitted parameter, not a claim that a passive Fresnel reflection coefficient
exceeds unity; the deployed smoothed values reported later remain below one.
...
\begin{equation}
\Gamma_{\perp} = \frac{\sin\theta_{\mathrm{g}} - \sqrt{\epsilon_c - \cos^2\theta_{\mathrm{g}}}}{\sin\theta_{\mathrm{g}} + \sqrt{\epsilon_c - \cos^2\theta_{\mathrm{g}}}},
\quad
\Gamma_{\parallel} = \frac{\epsilon_c\sin\theta_{\mathrm{g}} - \sqrt{\epsilon_c - \cos^2\theta_{\mathrm{g}}}}{\epsilon_c\sin\theta_{\mathrm{g}} + \sqrt{\epsilon_c - \cos^2\theta_{\mathrm{g}}}}
\label{eq:fresnel}
\end{equation}
where $\theta_{\mathrm{g}}$ is the grazing angle and
$\epsilon_c = \epsilon_r - j\sigma/(\omega\epsilon_0)$ is the complex
relative permittivity of the ground. Here $\epsilon_r$ is the relative
dielectric constant, $\sigma$ is the ground conductivity, $\omega=2\pi f$ is
the angular frequency, and $\epsilon_0$ is the vacuum permittivity. This
requires assuming physical constants for the ground (typically
$\epsilon_r \approx 15$ for urban soil).
The symbol is deliberately written as \(\theta_{\mathrm{g}}\) here because
the later A2G and morphology features use \(\theta(x)\) for the elevation
angle of the UAV to receiver link.
...
\label{eq:d4_law}
\end{equation}
Here \(P_t\) is the transmitted power, \(G_t\) and \(G_r\) are the transmitter
and receiver antenna gains, and \(P_r(d)\) is the received power at distance
\(d\).
...
a 60 m UAV at 7.125 GHz ($h_{\mathrm{rx}} = 1.5$ m). The plotted red curve uses
the code-loaded 60 m calibration values before adding the radial residual
profile, so it shows the constructive/destructive interference term rather than
the full final LoS prior.}
\label{fig:tworay_pl}
\end{figure}
...
Equation~\eqref{eq:cost231} should therefore be read as a calibrated
...
\item $+3$: the COST-231 metropolitan correction constant (dB), corresponding
      ...
      Eq.~\eqref{eq:cost231_a}. This deliberate mixture is not a canonical
...
formula map used by the NLoS calibration stage is assembled only after both
legacy branch estimates are available: the auxiliary LoS blend from
Eq.~\eqref{eq:los_blend}, and the conservative NLoS branch from
Eq.~\eqref{eq:nlos_raw}. This auxiliary LoS blend is not the final enhanced
two ray LoS prior; it is retained only because \(\mathrm{PL}^{(0)}\) is one
input feature of the calibrated NLoS branch.

The legacy LoS path term used by this auxiliary branch is:

\begin{equation}
\mathrm{PL}_{\mathrm{LoS,path}}(x)
=
\begin{cases}
\mathrm{FSPL}(x),
& d_{\mathrm{LoS}}(x) \le d_{\mathrm{c}},\\[2pt]
40\log_{10}\!\left(d_{\mathrm{LoS}}(x)\right)
-20\log_{10}(h_{\mathrm{tx}})
-20\log_{10}(h_{\mathrm{rx}}),
& d_{\mathrm{LoS}}(x) > d_{\mathrm{c}},
\end{cases}
\label{eq:legacy_los_path}
\end{equation}
...
=
\max\left\{
\frac{4\pi h_{\mathrm{tx}}h_{\mathrm{rx}}}{\lambda},
1~\mathrm{m}
\right\},
\qquad
\lambda = \frac{c}{f}.
\label{eq:legacy_crossover}
\end{equation}

Equation~\eqref{eq:legacy_los_path} switches from FSPL to the classical
asymptotic two ray \(d^4\) path loss law after the crossover distance
\(d_{\mathrm{c}}\). It is an older helper used only to construct the raw
\(\mathrm{PL}^{(0)}\) feature for the NLoS calibration, not the final enhanced
coherent two ray LoS prior of Eq.~\eqref{eq:pl_los}.

The legacy LoS blend is then defined as:

\begin{equation}
\mathrm{PL}_{\mathrm{A2G,LoS}}(x)
=
\lambda_0(h_{\mathrm{tx}}, f)
-20\log_{10}\!\left(\max\{\sin\theta(x),10^{-4}\}\right)
\label{eq:a2g_los}
\end{equation}

This auxiliary LoS envelope uses the same \(\lambda_0\) term as
Eq.~\eqref{eq:lambda0}; the sine clamp matches the implementation and avoids a
singularity at grazing elevation. It is a legacy helper for
\(\mathrm{PL}^{(0)}\), not the final LoS prior.
...
legacy LoS path model and 30\% weight to the lower-loss of the legacy path
model and the A2G LoS envelope. Because lower dB means a stronger received
signal, the \(\min(\cdot)\) operation should not be read as a conservative
guard against low path loss peaks; it is simply the older low-loss auxiliary blend inherited
by the raw \(\mathrm{PL}^{(0)}\) feature. Here
$\mathrm{PL}_{\mathrm{LoS,path}}(x)$ denotes the \emph{legacy} LoS path term
defined in Eq.~\eqref{eq:legacy_los_path}. It is not the calibrated enhanced
two ray map of Eq.~\eqref{eq:pl_los}.
...
it selects the auxiliary LoS expression on LoS pixels and the NLoS expression
on NLoS pixels. The auxiliary blend in Eq.~\eqref{eq:los_blend} was used in
older variants to keep the LoS formula below an A2G envelope. In the final
frozen path, LoS output pixels come directly from the separate enhanced
two ray prior \(\mathrm{PL}_{\mathrm{LoS}}\) of Eq.~\eqref{eq:pl_los}; the raw
hybrid map \(\mathrm{PL}^{(0)}\) in Eq.~\eqref{eq:pl0} is still computed only
as an input feature for the calibrated NLoS path loss branch.
...
The raw hybrid prior $\mathrm{PL}^{(0)}(x)$ defined in Eq.~\eqref{eq:pl0} is
...
feature vector $\mathbf{x}_{78}(x)$ defined below is \emph{not} a competing
...
propagation modelling, but adapted to the dense CKM setting. In particular, the
map morphology terms are inspired by the COST-231 Walfisch--Ikegami family,
where NLoS loss is not only a function of distance and frequency but also of
urban descriptors such as average building height, street width, building
separation and street orientation \cite{cost231,walfisch1988,ikegami1984}.
Here those hand-designed city descriptors are replaced by quantities that can
be computed directly and locally from the 513 by 513 topology map: windowed
building density, mean building height and local NLoS support. The elevation
and LoS/NLoS split follow the same physical intuition as A2G models, where
shadowing and blockage statistics vary strongly with elevation angle and
environment type \cite{alhourani2014,khawaja_survey}. The closest modern
machine-learning analogue is the path-profile work of Juang, which uses
building-height and street/profile statistics to explain urban path-loss
predictions with a linear regression surrogate \cite{juang2021pathprofile}.
The Try 78/80 NLoS branch keeps that explanatory spirit, but fits the
coefficients train-only on CKM pixels and applies the resulting calibration as
...
For the calibrated path loss prior, these two quantities are converted into
three coarse city types. This is a separate regime label from the later
six-class topology taxonomy used by the spread priors and by final
reporting:

\begin{equation}
\mathrm{class}_{78} =
\begin{cases}
\texttt{dense\_highrise} & \bar\rho \ge d_2\;\text{or}\;\bar h \ge h_2 \\
\texttt{open\_lowrise} & \bar\rho \le d_1\;\text{and}\;\bar h \le h_1 \\
\texttt{mixed\_midrise} & \text{otherwise}
\end{cases}
\label{eq:topo78}
\end{equation}

with thresholds $d_1 = 0.1957$, $d_2 = 0.2549$ (density fractions),
$h_1 = 10.91$ m, and $h_2 = 15.95$ m. Together with the binary LoS/NLoS
label and the three UAV height bins below, this yields the 18 stored path loss
calibration keys.
...
loads the stored coefficient vectors from
\texttt{try78\_nlos\_regime\_calibration.json}. At inference, if the exact
...
important object is the least squares coefficient vector stored in
\texttt{try78\_nlos\_regime\_calibration.json}; if a normal equation matrix is
rank deficient or ill conditioned, Eq.~\eqref{eq:ols_closed} should be
...
regime is:
\[
\textbf{w}^{78}_{\texttt{mixed|NLoS|high}}
=
\begin{bmatrix}
...
enhanced LoS two ray prior $\mathrm{PL}_{\mathrm{LoS}}$ reaches
\SI{1.7516}{dB}
pixel weighted RMSE on LoS pixels, the regime wise OLS NLoS head reaches
\SI{3.3967}{dB} on NLoS pixels, and the combined prior reaches
\SI{1.9246}{dB} overall on the held out validation+test split.
