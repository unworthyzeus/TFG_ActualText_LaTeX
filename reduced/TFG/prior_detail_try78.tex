...
The quantities \(\beta_{\mathrm{LoS}}(h_{\mathrm{tx}})\) and \(r(d_{2D}, h_{\mathrm{tx}})\) in
...
Table~\ref{tab:pl_los_term_inventory} gives the compact inventory of the
calibrated LoS terms. The individual distance and angle definitions are not
repeated here because they are shared support maps.
...
following 15 element vector is computed:
This follows the same intuition as urban NLoS models based on building rows and
