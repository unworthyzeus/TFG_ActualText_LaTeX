...
\emph{Gram matrix}: its diagonal and off-diagonal entries encode feature
energy and feature correlation inside regime \(r\). The vector
\(\textbf{X}_r^\top\textbf{y}_r\) encodes feature-target correlation. The
written inverse is full-rank ordinary least squares notation; the
implementation stores the least squares solution, using the corresponding
pseudoinverse solution if the normal equations are singular or ill conditioned.
...
their sizes mainly reflect correlated feature scales. The path loss prior is
therefore a calibrated surrogate: two-ray and A2G terms set the shape, while
per regime fitting adapts it to the CKM simulator and stored masks.
...
\(\mathrm{PL}^{(0)}(x)^2\) basis term. It is retained because this is the
stored result of the final expensive calibration run, although tests showed
that removing the feature or zeroing its coefficient does not meaningfully
change the path loss maps. A later calibration should drop it.
