...
city holdout protocol. The final system jointly predicts dense channel attenuation, delay
...
The final numbers meet the original targets. The channel attenuation prior reaches
\SI{1.9246}{dB} RMSE on the held out validation+test split and
\SI{1.9383}{dB} when used as the frozen prior on the final test split. The spread priors reach
\SI{28.10}{ns} DS RMSE and \(13.74^\circ\) AS RMSE on the same final test
split. \textsc{HARP-Net CKM} improves those RMSE anchors on 14
unseen test cities, reaching
...
\begin{itemize}
  \item a joint CKM generator for variable-height UAV channel attenuation, delay spread,
  ...
  \item a distribution first modelling lesson showing why GMM-style residual
        ...
        spread tails;
  \item a reproducible city holdout evaluation contract with valid ground
        receiver masking and LoS/NLoS/height diagnostics.
\end{itemize}
...
data while still reporting uncertainty by LoS/NLoS regime, transmitter height bin, and
topology class.
