...
Try 78 LoS branch then adds the fitted ground-reflection interference term,
...
\(\mathrm{clip}_{[20,180]}\) & Output guard. & Keeps the frozen prior inside the physical path-loss range used by Try 80. \\
\bottomrule
\end{tabularx}
\caption{Term-by-term inventory of the \texttt{Try 78} LoS prior used by
\texttt{Try 80}. Unlike the NLoS branch, this is not a long pixel-feature
...
\end{axis}
\end{tikzpicture}
\caption{Schematic comparison of FSPL and the enhanced two-ray model for a
...
\texttt{Try 78} variants to keep the LoS formula below an A2G envelope. In
the final frozen \texttt{Try 80} path, LoS pixels come directly from the
...
Eq.~\eqref{eq:pl0} is the only physical model in \texttt{Try 78}; it depends
...
\bottomrule
\end{tabularx}
\caption{Term-by-term inventory of the \texttt{Try 78} NLoS calibration
vector.}
...
two-ray calibration JSON used by \texttt{Try 80}, for example, is about
...
and NLoS regime coefficients. However, in the final frozen \texttt{Try 80}
...
This makes Try~78 an interpretable calibrated surrogate rather than a pure
...
The headline path-loss numbers for \texttt{Try 78} are reported in the
...
\texttt{Try 80} reaches \SI{1.9246}{dB} overall.
