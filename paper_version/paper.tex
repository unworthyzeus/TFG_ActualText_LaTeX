...
\urlstyle{same}
\hypersetup{
  pdftitle={HARP-Net CKM: Hybrid Deep Learning for Dense UAV Channel Knowledge Maps},
...
\title{\textsc{HARP-Net CKM}: Hybrid Deep Learning for Dense UAV Channel Knowledge Maps}
...
\textsc{HARP-Net CKM} (Height-Aware Residual Prior Network for Channel
Knowledge Map prediction), a hybrid surrogate that freezes physical and
statistical priors calibrated on the training split and learns bounded residual
maps with a probabilistic residual head. The model predicts \(513\times513\)
dense channel attenuation, DS, and AS maps from topology, geometric masks, five
frozen prior maps, and a separately encoded UAV transmitter height. A
probabilistic residual training phase teaches the head the residual
distribution, and a final deterministic fine tuning phase optimizes the point
maps used for RMSE reporting. On held out cities it reaches \SI{1.6519}{dB}
channel attenuation RMSE,
...
image regression problem. LoS attenuation has strong radial and height
structure, NLoS attenuation depends on blocked link regimes, and delay and
angular spread contain near zero LoS values with heavier NLoS tails. The
contribution is \textsc{HARP-Net CKM}, the Height-Aware Residual Prior Network
for Channel Knowledge Map prediction. It is a dense CKM recipe for one fixed
UAV evaluation setting: strict city holdout, valid receiver masking, explicit
LoS/NLoS and height reporting, priors calibrated only on the training split,
and a bounded neural correction around those priors.
...
frozen maps and predicts only bounded residual corrections. This division keeps
...
NLoS branch. More precisely, \eqref{eq:fspl-paper} is the Friis/free space
dB term and \eqref{eq:tworay-paper} is the standard coherent direct plus
...
\(\rho,\phi,\beta_{\mathrm{LoS}}\), and \(r\) curves are not taken from those references; they are
CKM training split calibration terms wrapped around that two ray structure. In
particular, \(\rho(h_{\mathrm{tx}})\) is an effective reflected field
amplitude and \(\phi(h_{\mathrm{tx}})\) is the fitted phase offset of that
reflected field relative to the direct ray; neither quantity is a material
Fresnel coefficient. The
...
\end{equation}
where \(\beta_{\mathrm{LoS}}\) is a height dependent bias and \(r\) is a smoothed radial residual
...
\(\beta_{\mathrm{LoS}}(h_{\mathrm{tx}})\), and \(r(d_{2D},h_{\mathrm{tx}})\) curves are fitted in
...
LoS reflection amplitude, whereas \(\delta_k\) denotes local building density;
similarly \(\beta_{\mathrm{LoS}}(h_{\mathrm{tx}})\) is a LoS bias, while \(b_q^{\mathrm{nat}}\),
...
The raw NLoS propagation prior is the maximum of two coarse branches: a
COST231 shaped urban loss branch and an A2G NLoS elevation envelope. These are
subterms of the raw blocked prior, not separate final models. The first term
below is the COST231/Hata urban macro formula adapted as a shape prior for the
CKM geometry, not as an assertion that the original model is directly valid at
every transmitter height:
\begin{equation}
\begin{aligned}
\mathrm{PL}_{C}
...
the calibration fitted on training cities correct the fixed excess loss
constants:
\begin{equation}
\begin{aligned}
...
\(\tau_{\mathrm{NLoS}}=7.52^\circ\). The calibration fitted on training cities
then bends these fixed constants to the CKM ray traced setting. The raw
blocked link prior is the conservative envelope
\begin{equation}
P_0
=
...
attenuation is kept. The retained raw attenuation is then corrected by a per
regime linear calibration fitted on training cities:
\begin{equation}
\mathrm{PL}_{\mathrm{NLoS}}
=\operatorname{clip}(\mathbf{x}_{\mathrm{NLoS}}^{\top}\mathbf{w}_{\mathrm{NLoS},r},20,180).
...
\begin{equation}
\begin{aligned}
\mathbf{x}_{\mathrm{NLoS}}=[&
P_0^2,P_0,\log(1+d_{2D}),\delta_{15},\delta_{41},\\
&h_{15},h_{41},\delta_{41}\log(1+d_{2D}),n_{15},\\
&n_{41},n_{41}\log(1+d_{2D}),\sigma_{\mathrm{shadow}},
\theta_{\mathrm{norm}},\\
&n_{41}\theta_{\mathrm{norm}},1]^{\top},
\end{aligned}
\label{eq:nlos-feature-vector-paper}
\end{equation}
where \(P_0\) is the raw NLoS prior, \(\delta_{15}\) and \(\delta_{41}\) are the
...
\(\delta_{41}\log(1+d_{2D})\) lets density matter differently with range,
...
jointly matter.
The constant \(1\) is a per regime bias term. The
...
\(\delta_k,h_k,n_k\) & NLoS PL and spreads & Topology and LoS/NLoS masks & Local density, building height, and NLoS support in a \(k\times k\) window. \\
...
\addlinespace[1pt]
\multicolumn{4}{l}{\emph{CKM training split fitted calibration}} \\
...
\(\beta_{\mathrm{LoS}}(h_{\mathrm{tx}})\) & LoS PL & Train split fit + smoothing & Smoothed deployed height dependent offset left after coherent two ray fitting. \\
...
\toprule
Target/regime & Starting structure & Training city calibration & Deployed prior map \\
\midrule
PL LoS &
FSPL plus coherent direct/reflected two ray correction
\cite{rappaport,goldsmith,wocc2021} &
\(\rho(h_{\mathrm{tx}})\), \(\phi(h_{\mathrm{tx}})\), \(\beta_{\mathrm{LoS}}(h_{\mathrm{tx}})\),
...
Per regime OLS over \(\mathbf{x}_{\mathrm{NLoS}}\), selected by morphology,
...
Ridge calibration over \(\mathbf{x}_{\mathrm{sp}}\) per metric/regime, with
...
\node[fit] (losfit) at (3.8,2.25) {LoS PL fit\\\(\rho,\phi,\beta_{\mathrm{LoS}}\) grid\\radial residual};
\node[fit] (nlosfit) at (3.8,1.25) {NLoS PL fit\\\(\mathbf{x}_{\mathrm{NLoS}}\) OLS\\regime fallbacks};
\node[fit] (spreadfit) at (3.8,0.25) {DS/AS fit\\\(\mathbf{x}_{\mathrm{sp}}\) ridge\\log domain};
...
\end{tikzpicture}}
\caption{Prior calibration and deployment. The upper row fits and
...
\begin{equation}
\hat\beta_{\mathrm{LoS}}(\hat\rho,\hat\phi)
...
\(\rho\), \(\phi\), and \(\beta_{\mathrm{LoS}}\) curves are then smoothed across neighbouring
height bins, weighted by bin support, and linearly interpolated at inference.

The NLoS branch uses the same training city principle but with a linear morphology
calibrator. For every supported regime \(r\), the calibration weights solve
\begin{equation}
\hat{\mathbf{w}}_{\mathrm{NLoS},r}
=
\operatorname*{arg\,min}_{\mathbf{w}}
\sum_{i\in r}
\left(\mathbf{x}_{\mathrm{NLoS},i}^{\top}\mathbf{w}-y_i\right)^2,
...
\eqref{eq:nlos-prior-paper}. This is an ordinary least squares objective,
...
&+a_3\delta_{41}+a_4h_{41}+a_5n_{41}
+a_6n_{41}\theta_{\mathrm{inv}},
\end{aligned}
\label{eq:spread-raw-paper}
\end{equation}
The constants in \eqref{eq:spread-raw-paper} are fixed raw prior
hyperparameters before ridge calibration. They come from the non neural spread
prior implementation rather than from the final residual network. Their role is
to give \(z^{(0)}\) a physically motivated starting scale: LoS and NLoS anchors
encode the spike plus tail split visible in CKM spread histograms, topology
offsets rank broad morphology classes, and positive weights on range, low
elevation, density, height, and NLoS support encode the expectation that richer
urban multipath increases spread. The ridge fit then learns the CKM specific
linear correction around this raw log prior. Numerically, for delay spread,
\(b_{\mathrm{LoS}}^{\mathrm{nat}}=\SI{4.0}{ns}\),
...
from map level building coverage \(\bar\delta_{\mathrm{map}}\) and mean
above ground building height \(\bar h_{\mathrm{map}}\): coverage
\(\bar\delta_{\mathrm{map}}\le 0.12\) is open sparse, with low rise if
\(\bar h_{\mathrm{map}}\le\SI{12}{m}\) and vertical otherwise; coverage
\(\bar\delta_{\mathrm{map}}\ge 0.28\) is dense block, with mid rise if
...
the prior for low elevation links, and \(\delta_{41}\), \(h_{41}\), and
...
&\delta_{15},\delta_{41},h_{15},h_{41},n_{15},n_{41},
n_{41}\log(1+d_{2D}),\\
&\delta_{41}\theta_{\mathrm{inv}},b_{41},1,c_{41},t_{41},
\theta_{\mathrm{norm}}\delta_{41},\\
&c_{41}\theta_{\mathrm{inv}},t_{41}\delta_{41},t_{41}n_{41}]^{\top}.
\end{aligned}
\label{eq:spread-feature-vector-paper}
\end{equation}
Here \(h\) is normalized transmitter height: one scalar per sample broadcast to
each valid receiver row. By contrast, \(b_{41}\), \(c_{41}\), and \(t_{41}\) are
\(41\times41\) map summaries: local blocker depth, transmitter clearance over
nearby buildings, and fraction of locally taller buildings. These height terms
are not incidental: they tell the spread prior whether the centred UAV is above
the local skyline, near rooftop level, or below nearby high rise clutter. The
...
\(\hat z=\mathbf{x}_{\mathrm{sp}}^{\top}\hat{\mathbf{w}}_{\mathrm{sp},r}\).
...
\(\mathbf{x}_{\mathrm{sp}}\). The point of this heavy calibration is to store many small
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth,height=0.62\textheight,keepaspectratio]{img/thesis_figures/harpnet_inspection_panels/harpnet_panel_dense_mid_ant_Vancouver_sample_15265_6x3.png}
...
rather than to a later interactive demo run. For traceability, the LaTeX thesis
source and project source archive are also available in public repositories
...
strict city holdout setting, the final model reaches \SI{1.6519}{dB},
...
strong priors fitted only on training cities for the structure that is
predictable, then learn bounded residual distributions around those anchors.
...
CKM simulation setting, \textsc{HARP-Net CKM} is
