...
mixed precision with batch size 1 and the vectorized CUDA LoS/NLoS backend
took \SI{1.22}{s} per map end to end. The component means were \SI{0.30}{s}
for LoS/NLoS mask generation, \SI{0.38}{s} for the frozen priors, and
\SI{0.54}{s} for the Try~80 neural forward pass. The mask-generation stage was
the previous bottleneck; after vectorization, the network forward pass is the
...
The resulting measured speed-up is \(82.0\times\) for the complete non-RT
generation path, including priors.

The mixed-precision numerical difference remains negligible. On the sampled
outputs, the RMSE between mixed precision and fp32 predictions was
\SI{0.0016}{dB} for path loss, \SI{0.0034}{ns} for delay spread, and
\SI{0.0030}{\degree} for angular spread, which is far below the validation RMSE
scale of the model.
