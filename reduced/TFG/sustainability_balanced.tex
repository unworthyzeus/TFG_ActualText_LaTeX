...
for the hardware were not audited, which is a limitation of the analysis. At the
end of life, the project mainly generates digital artefacts that can be archived
or deleted; no project specific physical waste is expected. If the work were
repeated, the footprint could be reduced by freezing the experiment plan earlier,
running smaller pilot subsets before full training, reusing checkpoints more
aggressively and scheduling heavy jobs when cleaner electricity is available.
...
contract used here, dense ray traced targets can be approximated much faster
than rerunning the simulator for every candidate deployment once the target city
representation and calibration protocol are fixed. The main development cost was human time:
...
studies over elevated transmitter heights or city layouts.
...
by LoS/NLoS state, transmitter height bin and morphology group rather than hidden behind
one global RMSE.
...
gender. From the project execution perspective, usability matters because the
tool would likely be used by engineers rather than by the general public: input
...
cultural background or limited ML expertise do not become hidden barriers. A
dependency risk also exists: if planners rely only on the surrogate and stop
running measurements or high fidelity simulation, users could become vulnerable
to systematic model errors.
...
station planning under the thesis dataset contract. This need was defined by the
...
cities, reporting limitations, and keeping the evaluation contract explicit.
