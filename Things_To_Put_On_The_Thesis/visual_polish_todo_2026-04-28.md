...
- And, finally, linear regression is what's used in try 78 and 79, right?
 ...
 Rename try 78 and 79 to correct name (based on how they predict), taking into account their 3 experts (topology classes) x3 (antenna height) x2 (LoS and nLoS, which are differently calibrated) (try 78) and 2 metrics x 6 experts x 3 antenna height x2 (LoS and nLoS), in total 72. And try 80 , even though it uses all these experts only reports 3 antenna heights x 3 experts. Maybe add why.