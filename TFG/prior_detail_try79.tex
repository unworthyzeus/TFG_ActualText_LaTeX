...
the fit and the near-zero LoS mass would be ignored.

\subsection{Pixel-wise notation for the Try 79 fit}
...
LoS/NLoS state and UAV-height bin. In other words, Try 79 does not fit one
...
\begin{equation}
z^{(0)}(x) = b_{\mathrm{LoS/NLoS}}(x) + b_{\mathrm{topology}} + a_1\log(1+d_{2D}) + a_2\theta_{\mathrm{inv}} + a_3\rho_{41} + a_4 h_{41} + a_5 n_{41} + a_6 n_{41}\theta_{\mathrm{inv}}
\label{eq:raw_spread}
\end{equation}

where $\theta_{\mathrm{inv}}(x) = 1 - \theta_{\mathrm{norm}}(x)$ (high value at low elevation). Example coefficients for delay spread: $b_{\mathrm{LoS}} = 4.0$, $b_{\mathrm{NLoS}} = 11.0$, $a_1 = 0.11$, $a_2 = 0.62$, $a_3 = 0.48$, $a_4 = 0.32$, $a_5 = 0.86$, $a_6 = 0.60$.
...
As in the Try 78 path-loss vector, some of these terms are not independent.
...
\bottomrule
\end{tabularx}
\caption{Term-by-term inventory of the direct terms in the \texttt{Try 79} spread calibration vector.}
...
\bottomrule
\end{tabularx}
\caption{Interaction terms in the \texttt{Try 79} spread calibration vector.}
...
\end{tikzpicture}
\caption{Feature grouping used by the \texttt{Try 79} calibrated spread prior.
...
\begin{equation}
h_{\mathrm{norm}} = \frac{\log\!\left(1+\max(h_{\mathrm{tx}}-h_{\mathrm{rx}},0)\right)}{\log(401)}
\label{eq:hnorm}
\end{equation}
This compresses UAV height into \([0,1]\) on a log scale, using
\(\log(401)\) as the normalizer for an expected maximum height difference of
about 400 m. The vector also includes \(h_{\mathrm{norm}}^2\), which is an
...
\begin{equation}
t_{41}(x) = \mathrm{BoxMean}_{41}\!\left(\mathbb{1}[\mathrm{topology}(\cdot)>h_{\mathrm{tx}}]\cdot b(\cdot)\right)(x)
\label{eq:t41}
\end{equation}
...
The regime $r$ for Try 79 is keyed by: metric (DS or AS), topology class (6 classes), propagation state (LoS or NLoS), antenna-height bin, giving up to $2 \times 6 \times 2 \times 3 = 72$ regimes.

Valid pixels are subsampled at rate $p = 0.02$ (2\%). The ridge regression:
\begin{equation}
\hat{\textbf{w}}^{79}_{r} = \left(\textbf{X}_r^\top \textbf{X}_r + \lambda \textbf{I}_{23}\right)^{-1}\textbf{X}_r^\top \textbf{z}_r,\quad \lambda = 10^{-3}
\label{eq:ridge}
\end{equation}
...
matrix for regime \(r\), \(\lambda\textbf{I}_{23}\) adds a small diagonal
...
large even after compression into fitted coefficients and profiles. The Try 78
LoS calibration JSON reused by Try 80 is approximately 200\,000 lines, and the
spread calibration must be rerun separately for delay spread and angular
spread whenever the feature set changes.

\paragraph{Fallback hierarchy.} If a regime has fewer than $4 \times 23 = 92$ pixels:
\begin{enumerate}
...
\end{tikzpicture}
\caption{Pipeline for the \texttt{Try 79} delay and angular spread priors. Both spread modalities share this identical linear architecture, predicting expected spread in the log domain via regime-wise ridge regression over 23 spatial and geometric features.}
