...
\textsc{HARP-Net CKM} follows the same strict split philosophy as the recent
...
\item calibrated NLoS path loss branch map,
\item delay spread prior,
\item angular spread prior.
\end{enumerate}

The seventh channel is named \texttt{path\_loss\_nlos\_prior} in the code,
but it should be read carefully. It is the calibrated mixed/NLoS path loss map
computed by the per regime path loss calibration, not a map that has been
zeroed outside NLoS pixels. In the final combined path loss prior and in the
final output selection, only the explicit LoS/NLoS masks decide which regional
branch is used.

The three mask channels are passed as binary maps. The topology channel is
masked to ground pixels and divided by \(90\,\mathrm{m}\); the three
path loss prior channels are divided by \(185\,\mathrm{dB}\); and the
delay and angular spread priors are passed as
\(\log(1+\mathrm{prior})/\log(1+910)\) and
\(\log(1+\mathrm{prior})/\log(1+90)\), respectively. These scaling constants
only affect the neural network input representation; the residual targets and
reported metrics remain in the native units. The path loss scale now matches
the physical output guard used by the priors and final assembly,
\([20,185]\) dB, so that the stored
\SI{184}{dB} valid receiver labels are not clipped.
...
conditioning. The selected final configuration uses a shared encoder decoder with
...
The selected final model reported in Chapter~\ref{chap:results}
...
\subsection[HARP-Net CKM Residual Head]{\textsc{HARP-Net CKM} Residual Head}

The final model introduces a joint multitask residual mixture head architecture that
emits a \emph{Gaussian Mixture Model (GMM)}-parameterized residual head at each
pixel, rather than only a direct scalar output. For each task \(k \in \{\mathrm{PL},\, \mathrm{DS},\, \mathrm{AS}\}\),
region \(r\in\{\mathrm{LoS},\mathrm{NLoS}\}\), component
\(j\in\{1,2,3\}\), and valid pixel \(x\), the model predicts a mixture
weight, a mean and a standard deviation. Channel attenuation is modeled in native dB
...
In the selected final fine tuning stage the same GMM parameterization is kept,
but the probabilistic losses are disabled; therefore the final reported claim
is based on the deterministic mixture mean map rather than on calibrated
...
where:
\begin{itemize}
\item \(y^{\mathrm{prior}}_{k}(x)\) is the per task frozen prior selected inside
      the shared output loop, then broadcast to the LoS and NLoS residual
      branches,
...
combines global and local standard deviation terms into the component standard
deviation:
\begin{equation}
\sigma_{k,r,j}(x)
=
...
\end{equation}
The Gaussian variance used by the density is therefore
\(\sigma^2_{k,r,j}(x)\). Both standard deviation terms are constrained by
sigmoid transforms to lie between \(\sigma_{\min}=0.05\) and
\(\sigma_{\max}=3.0\). These bounds stabilize the probabilistic head and
prevent the NLL term, when it is enabled, from collapsing to unrealistically
tiny uncertainty or exploding to a uselessly flat density.

The local component logits are transformed with a per pixel softmax, so
\(\sum_j p_{k,r,j}(x)=1\). The global mixture weights
\(\pi^{(g)}_{k,r,j}\) emitted by the global branch belong to the map level
distributional branch used when map level KL style terms are active; they are
not multiplied into the deterministic point map in \eqref{eq:try80_point}.
This distinction is important for the selected final checkpoint, where the
distributional losses are disabled and RMSE is computed from the local weighted
point prediction.

The architecture supports a full GMM Negative Log Likelihood (NLL), and the
preceding large capacity residual stage used nonzero probabilistic weights.
The selected final model keeps the same GMM head, but the last residual
fine tuning stage is RMSE/MAE dominant: its NLL, KL, moment and
outlier budget weights are set to zero in the resolved YAML configuration.
The deterministic point prediction used for RMSE evaluation is the mixture
mean in the modelled space:
...
\end{equation}
For path loss, this is also the native space mixture mean because the modelled
space is dB. For delay spread and angular spread, the mixture is averaged in
\(\log(1+y)\) space and then mapped back with \(\exp(\cdot)-1\). Therefore the
reported point map should be read as the deterministic prediction used for
RMSE, not as the exact native space expectation of the probabilistic mixture.
...
The final model processes the map inputs through a shared multitask
convolutional encoder decoder. In the final large capacity configuration,
...
The FiLM mechanism and injection points are unchanged between the wide and
large capacity variants; only the width of the feature maps changes, so the
FiLM projection emits \(2C\) parameters for the larger channel counts.

The word ``joint'' in this model refers to shared representation learning and
joint optimization, not to an autoregressive or cross output decoder. Path loss,
delay spread and angular spread all pass through the same encoder, the same
height conditioning path, the same decoder feature map and the same
\textsc{HARP-Net CKM} residual assembly rule. After these shared features are computed,
however, the implementation iterates over the three tasks and applies
task specific global and local heads. Therefore PL, DS and AS can exchange
information through the common spatial representation, but the final residual
parameters, mixture weights and frozen prior anchor are selected independently
for each task.

The architecture separates global and local effects with two task specific
heads. The \textbf{global head} average pools the deepest bottleneck feature
map, fuses it with the height condition, and predicts map level distributional
quantities: auxiliary mixture weights \(\pi^{(g)}_{k,r,j}\), offsets
\(\delta^{(g)}_{k,r,j}\), and standard deviations \(\sigma^{(g)}_{k,r,j}\).
The \textbf{local head} applies \(1\times1\) convolutions to the decoder output
and predicts spatial fields \(p_{k,r,j}(x)\), \(\delta^{(\ell)}_{k,r}(x)\),
\(\alpha_{k,r}(x)\), and \(\tilde\sigma_{k,r}(x)\).
...
% Prior Input
\node[align=center, draw, thick, fill=gray!20, rectangle, minimum height=0.8cm] (prior) at (14, -3) {\textbf{Per task Frozen Prior} \\ selected in shared loop \\ $y_k^{\mathrm{prior}}(x)$};

% GMM Assembly
\node[gmm] (gmm) at (10, -6) {\textbf{GMM Assembly} \\[0.2cm]
$\Delta_{k,r,j}=\mathrm{clip}(\delta^{(g)}_{k,r,j}+\delta^{(\ell)}_{k,r},\,\pm\delta_{\max,k,r})$ \\
$\mu_{k,r,j} = y_k^{\mathrm{prior}} + \alpha_{k,r}\Delta_{k,r,j}$ \\
$\sigma^2_{k,r,j} = \sigma^{(g)2}_{k,r,j} + \tilde\sigma_{k,r}^2$ \\[0.2cm]
$\hat z_{k,r} = \sum_j p_{k,r,j} \mu_{k,r,j}$\\
PL: $\hat y=\hat z$; DS/AS: $\hat y=\exp(\hat z)-1$};

% Routing Prior to GMM
\draw[->, thick] (prior.south) |- node[pos=0.25, right, font=\scriptsize] {$\mathbb{R}^{1 \times H \times W}$} ([yshift=0.5cm]gmm.east);

% Pixel Level outputs flowing to GMM
\draw[->, thick] (local_head.south) -- node[right, align=left, font=\scriptsize] {$p_{k,r,j}(x)$ \\ $\alpha_{k,r}(x)$ \\ $\delta^{(\ell)}_{k,r}(x)$ \\ $\tilde\sigma_{k,r}(x)$} (gmm.north);

% Map Level outputs flowing to GMM
\draw[->, thick] (global_head.east) -| node[pos=0.25, above, align=center, font=\scriptsize] {$\pi^{(g)}_{k,r,j}$ \\ $\delta^{(g)}_{k,r,j}$ \\ $\sigma^{(g)}_{k,r,j}$} ([xshift=-1cm]gmm.south);

% Final Output
\node[align=center] (final_out) at (14, -8.5) {\textbf{Final Prediction} \\ $\hat y_k(x)$};
\draw[->, thick] ([yshift=-0.5cm]gmm.east) -| node[pos=0.75, right, font=\scriptsize] {$\mathbb{R}^{1 \times H \times W}$} (final_out.north);

\end{tikzpicture}
}
\caption{Schematic of the final large capacity shared UNet architecture with FiLM conditioning. The backbone and decode logic are shared, while each task uses its own heads and frozen prior.}
...
\node[featbox] (pool) at (18.0,-3.5) {Global average pool\\deepest map\\$\mathbb{R}^{512\times H_{16}\times W_{16}} \to \mathbb{R}^{512}$\\first 160 entries feed fusion};
...
\draw[arr] (pool.north) -- node[right, font=\scriptsize] {$\mathbb{R}^{160}$} (fuse.south);
...
\paragraph{Detailed HARP-Net CKM residual head.}
For each task
...
$\tilde\sigma_{k,r}(x)$.
After softmax over components, \(p^{(\ell)}_{k,r,j}(x)\) becomes the local
probability \(p_{k,r,j}(x)\) used in the regional model space mean. The auxiliary
global weights \(\pi^{(g)}_{k,r,j}\) are retained for the distributional
training branch and diagnostics, but the selected RMSE/MAE dominant checkpoint
uses the local probabilities for the deterministic output map.
...
\node[prior] (prior) at (0,-6.5) {Frozen task prior\\broadcast to regions\\$y^{\mathrm{prior}}_{k}(x)$};
\node[mix] (mu) at (17.0,-6.5) {Anchored means\\native residual, then task transform\\$\mu_{r,j}(x)$};
\node[mix] (sigma) at (21.5,-3.0) {Std.-dev. merge\\$\sigma_{r,j}(x)=\sqrt{(\sigma^{(g)}_{r,j})^2+(\tilde\sigma_r(x))^2}$};
...
\node[mix] (expv) at (17.0,-10.0) {Regional model space mean\\$\hat z_{k,r}(x)=\sum_{j=1}^{3} p_{k,r,j}(x)\,\mu_{k,r,j}(x)$\\PL: $\hat y=\hat z$; DS/AS: $\hat y=\exp(\hat z)-1$};
...
\caption[Detailed HARP-Net CKM residual head used by the final model.]
{Detailed \textsc{HARP-Net CKM} residual head used by the final model.}
...
\(\Omega_k\) be the valid ground pixel set, let \(q_k(z\mid x)\) be the
predicted mixture density in the modelled space, with \(z=y\) for path loss and
\(z=\log(1+y)\) for delay spread and angular spread, and let \(\hat y_k(x)\) be
the native space point prediction used for RMSE. The total objective is:
...
\begin{table}[!htbp]
\centering
\caption[Active loss settings in the HARP-Net CKM stages.]
{Active loss settings in the \textsc{HARP-Net CKM} stages.}
...
large capacity residual stage and the final residual fine tuning stage; the
equations below define the terms used in that table.

\paragraph{Map negative log likelihood.}
The likelihood is computed separately in LoS and NLoS regions, then averaged
over the nonempty regions. For a transformed target \(z_k(x)\),
\begin{equation}
...
\label{eq:try80_nll}
\end{equation}
The implemented version evaluates \eqref{eq:try80_nll} using
...
\end{equation}
The guard term is \eqref{eq:try80_loss}. Together, these two terms encode
...
In \eqref{eq:try80_anchor}, the residual variables are the same tensors
used by the implementation: native dB offsets for path loss, and the
corresponding \(\log(1+y)\)-space offsets for delay and angular spread after
the native residual caps have been applied. The guard term, by contrast, is
evaluated in the native units of each task against the frozen prior map.

\paragraph{Outlier budget and deterministic losses.}
When the outlier budget term is active, the implementation uses the last local
mixture component as a proxy for broad/outlier usage. This is an indexing
convention rather than a hard architectural constraint: the model is not forced
to make that component the broadest one. If the mean usage of this proxy exceeds
\(\tau=0.60\), a hinge penalty is applied:
\begin{equation}
...
loss family. In the final residual fine tuning stage, the selected final model
keeps the GMM head and uses RMSE/MAE, anchor and guard penalties; NLL, KL,
moment and outlier budget weights are set to zero. Earlier residual stages
used nonzero probabilistic weights.}
