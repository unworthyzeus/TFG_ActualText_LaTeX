...
For reading, the vector in Eq.~\eqref{eq:x79} can be grouped into five blocks:
...
The spread prior uses the six topology labels produced by the final
prior-generation code. For a map, let \(\bar\rho_{\mathrm{map}}\) be the fraction of
...
\node[block, below=1.5cm of inputs] (raw_prior) {Stage 1: Raw Log Prior \\ $z^{(0)}(x)$};
