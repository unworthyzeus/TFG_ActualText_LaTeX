...
10^8)) \approx 32.45$ dB. In \eqref{eq:fspl}, $d_{\mathrm{LoS}}(x)/1000$
...
(\eqref{eq:field_sum}) without the $d^4$ approximation. This is still a
...
\eqref{eq:los_blend}, and the conservative NLoS branch from
\eqref{eq:nlos_raw}. This auxiliary LoS blend is not the final enhanced
...
\eqref{eq:legacy_los_path} switches from FSPL to the classical
...
coherent two ray LoS prior of \eqref{eq:pl_los}.
...
\eqref{eq:lambda0}; the sine clamp matches the implementation and avoids a
...
The $0.7/0.3$ blend in \eqref{eq:los_blend} gives 70\% weight to the
...
defined in \eqref{eq:legacy_los_path}. It is not the calibrated enhanced
two ray map of \eqref{eq:pl_los}.
...
Since $m_{\mathrm{LoS}}(x)$ is binary, \eqref{eq:pl0} is a hard switch:
it selects the auxiliary LoS expression on LoS pixels and the NLoS expression
on NLoS pixels. The auxiliary blend in \eqref{eq:los_blend} was used in
...
hybrid map \(\mathrm{PL}^{(0)}\) in \eqref{eq:pl0} is still computed only
...
\eqref{eq:boxmean} is evaluated efficiently using a
...
Since $n_{\mathrm{raw}}(x') \in \{0,1\}$ (\eqref{eq:nlos_mask_v2}),
...
\eqref{eq:h_k_interp}, which divides the same numerator by the full
...
\end{bmatrix}^\top
\label{eq:try78_mixed_midrise_high_coefficients}
\end{equation}
