...
unseen data. In city holdout radio map prediction, this idea is relevant because
the validation and test cities are never seen during training.

\subsubsection{Exponential moving average weights}

Separate from SWA, exponential moving average (EMA) tracking of the model
weights can produce smoother validation checkpoints by reducing short term
training noise. EMA is therefore often discussed together with plateau based
learning rate schedules and checkpoint selection, although it is a training
stabilisation tool rather than a propagation model.
