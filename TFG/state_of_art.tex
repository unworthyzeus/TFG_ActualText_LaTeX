...
paper \cite{icassp2023challenge,dataset2212}.}
...
silhouette and converted to a diffraction gain.  Providing this as an extra
input channel was shown to yield up to $18\,\%$ accuracy improvement in
geometry-assisted deep learning experiments.  Try~67 of this thesis implements
this channel through the \texttt{knife\_edge.py} module, feeding it into the
PMHHNet encoder alongside the topology map.
...
Try~78 implements this philosophy for path loss: a coherent two-ray model fitted
...
Mixture models are the natural solution.  GMM-3 was identified as the best-fit
family for every LoS/NLoS histogram in the CKM distribution analysis.  The
mixture component with the lowest mean captures the spike, and the remaining
components capture the exponential tail.  This motivates the Stage-A distribution
head in Try~76 (for path loss) and Try~77 (for spreads), carried into Try~80 as
the distributional prior for the residual head.
...
overestimating uncertainty everywhere.  Try~71 of this thesis applies this loss
to path loss prediction, with $\sigma^2$ predicted as a second output channel of
PMHHNet.  Evaluation curves of RMSE versus uncertainty-threshold coverage showed
that filtering the highest-uncertainty pixels substantially reduces overall RMSE.

\subsection{Gaussian mixture model heads}

Beyond heteroscedastic Gaussians, the GMM head allows multi-modal output
distributions.  Try~76 implements a two-stage architecture: Stage~A produces
per-sample GMM parameters $\{\pi_k, \mu_k, \sigma_k\}_{k=1}^{K}$ via a shallow
convolutional encoder with sinusoidal FiLM conditioning on UAV height; Stage~B
is a small U-Net that produces per-pixel component assignments $p_k(i,j)$ and
...
spike-plus-exponential prior structure.
...
angular-spread targets.  I therefore use it as an outdoor path-loss reference
for the approximate difficulty of real-environment generalisation, while keeping
the final comparison qualitative.
...
emphasises the direct Tx--Rx corridor.  The authors report that this design
outperforms PPNet, RPNet, and ViT baselines by roughly \(1.2\)--\SI{3.0}{dB}
on ITU Challenge 2024 and ICASSP~2023-style data, while reducing FLOPs by about
60\%.  This is highly relevant to the CKM work because it formalises a point
...
\begin{table}[ht]
\centering
\caption{Emerging techniques ranked by expected impact on CKM NLoS RMSE.}
\label{tab:emerging}
\begin{tabularx}{\textwidth}{clXcc}
\toprule
\textbf{\#} & \textbf{Technique} & \textbf{Key reference} & \textbf{Est.\ gain} & \textbf{Complexity} \\
\midrule
1 & Knife-edge diffraction prior & \cite{geomDL2024} & 1--3\,dB NLoS & Medium \\
2 & PINN Helmholtz residual & ReVeal \cite{reveal2025} & 0.5--2\,dB & High \\
3 & Dual LoS/NLoS heads & Tries~69, 71 & 1--2\,dB & Low \\
4 & Test-time adaptation & RadioPiT \cite{radiopit2025} & 0.3--1\,dB & Low--Med \\
5 & Diffusion refinement & RadioDiff \cite{radiodiff2025} & 0.5--2\,dB & High \\
6 & Foundation pre-training & FM-RME \cite{fmrme2026} & 0.3--1\,dB & Very high \\
\bottomrule
\end{tabularx}
\end{table}

Items~1--3 are partially implemented in the current thesis; items~4--6 are
roadmap items requiring additional data, compute, or infrastructure beyond the
scope of a single-GPU training setup.
...
Try~80 occupies a distinct position in the radio-map literature through the
combination of three features that no published method addresses simultaneously:
...
\begin{itemize}
  ...
  \item \textbf{Frozen physics anchors.}  Two independently calibrated
        physics-based priors~--- the coherent two-ray model (Try~78) for path
...
\toprule
\textbf{Benchmark / Method} & \textbf{True RMSE (path loss)} & \textbf{City holdout} & \textbf{Verdict} \\
\midrule
ICASSP~2023 best \cite{rmtransformer2025} & 0.97\,dB & No & Different problem \\
AIRMap \cite{airmap2025} (calibrated) & $<5$\,dB & Partial & Different protocol \\
Gao et al.\ \cite{gao2026} (multi-layer) & 5.6\,dB & No & Different protocol \\
Tarhouni et al.\ \cite{tarhouni2025} & multi-dB, sub-6\,GHz & No & Physical outdoor reference \\
Try~66 (this thesis, single expert) & 9.41\,dB & Yes & Internal baseline \\
Try~78 non-DL prior & $\approx$1.75\,dB LoS & Yes & LoS-only anchor \\
\textbf{Try~80 (this thesis)} & \textbf{target $<$5\,dB} & \textbf{Yes} & \textbf{This work} \\
\bottomrule
\end{tabularx}
\end{table}

The $<5\,\mathrm{dB}$ path-loss target for Try~80 is framed as a project goal
rather than a published result, consistent with the thesis performance targets
stated in the introduction.  The prior-anchored design is expected to allow
the neural residual head to converge faster and to a narrower error floor than
the direct-regression family of Tries~66--71, particularly in LoS-dominated
