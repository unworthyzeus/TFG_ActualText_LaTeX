...
\cite{pmnet2023,pmnet_icassp2023,icassp2023challenge}. More recent models introduce attention,
...
\cite{pathfinder2025}, Gao et al.'s corridor weighted multilayer
...
A2G path loss and delay spread with classical machine-learning regressors
\cite{yang2019a2gml}, while Huang et al.\ use a Transformer style predictor
for UAV mmWave received power, RMS delay spread, and RMS angular spread from
link level geometric features \cite{huang2025a2gtransformer}. Mi et al.\
predict mmWave path loss, RMS delay spread, angular spread, and Rician \(K\)
factor from measurement-linked point clouds \cite{mi2024pointcloud}. These
papers are close in target type, but not in evaluation contract: they predict
link level or sparse channel parameters, not full \(513\times513\) CKM maps
for unseen cities.
...
closest external references are often scalar or link level channel parameter
predictors and standard channel model statistics \cite{tr38901,winner2,cai2019,
izydorczyk2019}, not dense maps, so they are not used as direct CKM
benchmarks here.

\section{Dataset and Task Contract}
...
\begin{table}[!t]
\centering
\caption{Input/output contract of the full framework and final neural model.}
\label{tab:io-contract-paper}
\scriptsize
\setlength{\tabcolsep}{3pt}
...
Building-height map and UAV transmitter height. \\
...
Topology map, UAV height conditioning, ground mask, LoS mask, NLoS mask,
and the five frozen prior maps. \\
Final outputs &
Path-loss, delay spread, and angular spread maps. \\
\bottomrule
\end{tabularx}
\end{table}

Thus the full generator has two external inputs and three final outputs. The
neural residual model has ten internal inputs because the framework has already
computed the visibility mask, receiver mask, and frozen prior maps.
...
interpreted as the numerical least squares or pseudoinverse solution associated
with this objective.
...
\texttt{high\_ant}: \(h_{\mathrm{tx}}>\SI{103.85}{m}\)); the spread model
also keeps continuous height, clearance, and taller building features inside
\(\mathbf{x}_{79}\). These calibration bins are distinct from the broader
height-reporting bins in Tables~\ref{tab:pl-prior-height} and
\ref{tab:spread-prior-height}. This heavy calibration is the point: the
...
not as a calibrated probabilistic predictor. In this final stage, checkpoint
...
\subsection{Main Test Results by Target}
...
is the cleanest success case because both RMSE and MAE improve. Angular spread
also improves strongly in RMSE and remains approximately neutral in MAE. Delay
spread improves RMSE while MAE slightly increases; this is consistent with
reducing rare high delay errors while introducing small local shifts.
...
reduces these to \SI{1.6519}{dB} and \SI{0.9298}{dB}. This is the most
...
The angular spread prior reaches \(13.7416^\circ\) RMSE and \(4.9890^\circ\)
MAE; the final model reaches \(11.3854^\circ\) RMSE and \(5.0088^\circ\) MAE.
The near-constant MAE means the residual model mainly suppresses larger angular
...
papers predict link level or sparse angular spread channel parameters
\cite{huang2025a2gtransformer,mi2024pointcloud}, so the primary comparison is
against the frozen prior and the thesis target.

\paragraph{Delay spread}
The delay spread prior reaches \SI{28.1023}{ns} RMSE and \SI{7.3033}{ns} MAE;
the final model reaches \SI{26.5570}{ns} RMSE and \SI{7.9897}{ns} MAE. This
...
\cite{yang2019a2gml,huang2025a2gtransformer}, but they are not dense
\(513\times513\) CKM generators under city holdout, so they are used only as
context.
...
The validation/test sanity check is also useful. The validation row reports an
external re evaluation of the selected checkpoint, while the test row is the
real generalization result. Delay spread worsens more on test because the
...
split because Try~79 shares the 10840/2750/2590 train/validation/test contract
used by the final Try~80 model.
...
\begin{table}[!t]
\centering
\caption{Calibrated path loss prior RMSE by UAV height on the held out
...
\begin{table}[!t]
\centering
\caption{Calibrated spread prior RMSE by UAV height on the final 14-city
test split.}
...
(\texttt{low}, \texttt{mid}, \texttt{high}), using the same
\SI{58.12}{m} and \SI{103.85}{m} thresholds as the frozen prior
calibration keys. These rows are not nine separately
...
\texttt{open/low} & 1269 & -0.2363 & -3.1170 & -2.0271 \\
\texttt{open/mid} & 1299 & -0.2941 & -1.5789 & -1.7804 \\
\bottomrule
\end{tabular}
\end{table}
...
frozen priors then provided stronger anchors: the standalone path loss prior
reached \SI{1.9246}{dB} RMSE on its held out validation+test split and
\SI{1.9383}{dB} when frozen on the final test split, while the spread priors
reached \SI{28.10}{ns} delay spread RMSE and \(13.74^\circ\)
angular spread RMSE on that same final test split.
...
\textbf{Final model} &
\textbf{\SI{26.5570}{ns} DS RMSE and \(11.3854^\circ\) AS RMSE.} &
\textbf{Dense PL/DS/AS maps under the final city holdout contract.} &
...
results now have explicit nearby literature context in
Table~\ref{tab:spread-soa}, but should still be interpreted primarily against
same protocol internal baselines because no external dense CKM PL/DS/AS
benchmark was found.
...
speedup under this measurement setup. The ray tracing row uses the CPU path;
...
many planning experiments, achieving a measured \(113.7\times\) speedup over
...
structure that the priors cannot explain.
