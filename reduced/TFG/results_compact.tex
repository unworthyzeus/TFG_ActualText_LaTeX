...
Frozen priors & Train-only calibrated priors became strong baselines in their own right. & \SI{1.9246}{dB} standalone PL prior; \SI{28.10}{ns} DS; \(13.74^\circ\) AS. \\
...
\SI{1.9246}{dB} overall on its 30\% analytic-prior evaluation holdout
...
\begin{table}[htbp]
\centering
\caption{Standalone calibrated path-loss prior on its 30\% city-holdout evaluation split.}
...
\begin{table}[htbp]
\centering
\caption{Compact height audit. Values are prior-only RMSE.}
\label{tab:height_topology_audit}
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrrr}
\toprule
Height bin & PL prior & DS prior & AS prior & Samples \\
\midrule
