...
conditioning. The final \texttt{try80\_joint\_huge\_relaxed\_residuals}
...
\texttt{Try 80} processes the map inputs through a shared multi-task
...
\caption{Schematic of the final \texttt{Try 80 huge} shared UNet architecture with FiLM conditioning.}
...
The numerical weights used in the larger
\texttt{try80\_joint\_huge\_relaxed\_residuals} variant are
...
\(\lambda_{\mathrm{RMSE}}=1.50\), and \(\lambda_{\mathrm{MAE}}=0.10\).
The base \texttt{try80\_joint\_big} configuration described earlier uses
\(\lambda_{\mathrm{RMSE}}=0.40\); the difference is a deliberate
relaxed-residual hyperparameter change, not a notation change.
