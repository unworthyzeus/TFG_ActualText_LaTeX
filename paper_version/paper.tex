...
dense UAV CKM generation remains difficult when path loss, delay spread, and
...
mixture head. The model predicts \(513\times513\) dense PL/DS/AS maps from
...
tuning. On held out cities it reaches \SI{1.6519}{dB} path loss RMSE,
...
family showed that U-Net-like models can be strong dense PL baselines
...
gao2026,airmap2025}. These papers give the closest dense PL context, but not
the same contract: continuous UAV height, strict city holdout on CKM samples,
and three dense targets (PL, DS, AS).
...
predict mmWave PL, DS, AS, and \(K\)-factor from measurement-linked point
