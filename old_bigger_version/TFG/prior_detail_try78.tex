...
used by Try~80 are the smoothed $\rho$, $\phi$, and $b$ curves themselves, not
...
earlier variants to keep the LoS formula below an A2G envelope. In the final
frozen path, LoS pixels come directly from the separate enhanced two-ray prior
\(\mathrm{PL}_{\mathrm{LoS}}\) of Eq.~\eqref{eq:pl_los}; the raw hybrid map
\(\mathrm{PL}^{(0)}\) in Eq.~\eqref{eq:pl0} is still computed as an input
feature for the calibrated Try~78 NLoS branch.
...
the scalar formula input to the calibrated Try~78 NLoS branch. It combines the
...
For the Try~78 path-loss calibration, these two quantities are converted into
three coarse city types. This is a separate regime label from the later
six-class topology taxonomy used by the Try~79 spread priors and by final
reporting:
...
label and the three UAV-height bins below, this yields the 18 stored Try~78
...
Try~78 city type and LoS/NLoS label. It does not use the broader cross-topology
fallback hierarchy of the Try~79 spread-prior calibration.
