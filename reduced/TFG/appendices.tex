...
78 & Final ML-calibrated path loss prior: enhanced two ray LoS branch plus a legacy mixed branch linear calibration table with 18 visibility/morphology/height regimes, from which the final assembly uses the NLoS pixels. \\
...
80 & \textsc{HARP-Net CKM}: frozen path loss and spread priors plus a shared neural residual model selected through the wide residual model, relaxed residual bounds, a large capacity stage, and the final residual fine tuning stage. \\
\bottomrule
\end{tabularx}
\end{table}
\normalsize
