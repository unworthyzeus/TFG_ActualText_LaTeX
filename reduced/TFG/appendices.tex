...
\textsc{HARP-Net CKM} beats the frozen prior on all three outputs, then chooses the
...
{\textsc{HARP-Net CKM} appendix panels generated from selected validation/test samples with the
...
dense/mid & Vancouver & sample\_15265 & mid \\
...
\caption{\textsc{HARP-Net CKM} \(6\times3\) inspection panel for an open, mid-antenna
validation sample in Segovia.}
...
\caption{\textsc{HARP-Net CKM} \(6\times3\) inspection panel for a mixed, low-antenna
...
\caption{\textsc{HARP-Net CKM} \(6\times3\) inspection panel for a mixed, high-antenna
validation sample in Nice.}
...
\caption{\textsc{HARP-Net CKM} \(6\times3\) inspection panel for a dense, low-antenna
...
\caption{\textsc{HARP-Net CKM} \(6\times3\) inspection panel for a dense, mid-antenna
...
\caption[Full non residual DL GMM head model for PL.]%
{Full non residual DL GMM head model for path loss (PL): five-component
Gaussian-mixture fits of the LoS and NLoS path-loss targets,
...
The dashed lines mark the per-expert LoS and NLoS marginal means.}
...
\textsc{HARP-Net CKM} final attempts. The binned curves are lightly smoothed for
readability.}
...
{\textsc{HARP-Net CKM} path loss histograms on the same
...
and \textsc{HARP-Net CKM}. The path loss panel uses global
...
{Mean component timing for the \textsc{HARP-Net CKM} generator on the same ten
...
\textsc{HARP-Net CKM} neural prediction for PL, DS and AS, plus default \texttt{.npz} export & \SI{0.54}{s} & \(61.2\%\) \\
\midrule
...
Figure~\ref{fig:try80_panel} shows a representative final-model held out test
...
mid-antenna held out test sample in Vancouver. Columns: path loss, delay
