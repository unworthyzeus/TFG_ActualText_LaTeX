...
systems in which UAVs or other elevated radio access nodes may act as aerial
base stations, relays, or temporary coverage nodes. Ray tracing can produce
detailed channel knowledge maps, but it
is too expensive to repeat for every transmitter height, city morphology or
deployment hypothesis. This thesis studies whether a hybrid predictive model can
predict dense channel maps directly from urban geometry while preserving enough
physical structure to generalize to cities not seen during training. The CKM
files name the transmitter as a UAV, but the same fixed centre
\SI{7.125}{GHz} contract also represents elevated base station planning within
the sampled height range.

The project is formulated as dense prediction on \(513\times513\) city maps.
Inputs include building-height topology, geometric LoS/NLoS masks and the
transmitter antenna height. The outputs are channel attenuation, delay spread and angular spread. The
...
channel attenuation RMSE below \SI{5}{dB}, delay spread RMSE below \SI{50}{ns},
...
a height aware calibrated channel attenuation prior, log domain calibrated DS/AS priors,
and \textsc{HARP-Net CKM}, which improves those frozen anchors.
...
at \SI{1}{m}/pixel. The elevated transmitter is horizontally centred, but its
...
topology experts improved LoS but not NLoS tails; distribution aware diagnostics
...
methodology, including the channel attenuation and spread priors and
\textsc{HARP-Net CKM}. Chapter~4 reports final unseen-city results, LoS/NLoS diagnostics,
