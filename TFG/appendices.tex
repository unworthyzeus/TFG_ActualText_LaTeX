...
80 & Final joint prior-anchored residual GMM-head model: frozen Try 78/79 priors plus a shared neural residual model selected through Big \(\rightarrow\) Big relaxed residuals \(\rightarrow\) Huge \(\rightarrow\) Huge path-loss fine-tuning. \\
\bottomrule
\end{tabularx}
\end{table}
\normalsize
...
Tries 1--8 treated CKM prediction as aligned image-to-image regression on the
...
Tries 9--14 introduced explicit UAV-height conditioning and LoS/NLoS-aware
...
Tries 15--22 then attacked visible artifacts and missing large-scale structure.
...
Tries 33--40 are important mainly because they changed the metric contract.
...
Tries 41--50 moved from direct prediction toward residual learning over a
...
Tries 51--66 expanded the residual idea into PMHHNet and topology-expert
...
Tries 67--75 tested whether late training refinements could rescue the
...
output design even when the final checkpoint is selected by deterministic
validation RMSE.
...
therefore moved through Big, Big with relaxed residuals, Huge with relaxed
residuals, and Huge path-loss fine-tuning. The last stage kept the large
architecture and relaxed caps, but used an RMSE-dominant fine-tune with light
anchor and prior-guard terms. This is the checkpoint reported in
...
the validation split, the final huge path-loss-finetuned checkpoint, DirectML
...
\begin{table}[H]
\centering
\caption{Final residual GMM-head model appendix panels generated from validation samples with the
final huge checkpoint.}
...
timing only model inference on an RTX 3050 Ti Laptop GPU, full fp32 took
