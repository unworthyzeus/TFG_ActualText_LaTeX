...
quantitative claim; this appendix keeps the full numbering context. Equations
and metric tables are not duplicated here: Chapter~3 contains the modelling
definitions and Chapter~\ref{chap:results} contains the quantitative results.
...
80 & Final joint prior-anchored residual GMM-head model: frozen Try 78/79 priors plus a shared neural residual model selected through the wide residual model, relaxed residual bounds, a large-capacity stage, and the final residual fine-tuning stage. \\
\bottomrule
\end{tabularx}
\end{table}
\normalsize
...
Try 76 reframed NLoS path loss as a distributional problem. Instead of forcing
one value everywhere, Stage A estimated sample-level mixture structure and
Stage B placed components spatially. This produced much stronger NLoS
behaviour, with the DirectML summary around \SI{3.34}{dB} pixel-weighted RMSE.
The key lesson was that a model can approach the hard regime by estimating the
right distribution before assigning pixels.

Try 77 transferred the same philosophy to delay and angular spread. It was
not the final deployed spread baseline, but it explained why spread targets
should not be treated as ordinary smooth images. Together, Tries 76 and 77 are
the conceptual bridge to the GMM residual head retained in the final attempt.

\subsection{Frozen calibrated priors}

Try 78 showed that LoS path loss was more deterministic than the neural
...
Try 79 applied the same estimator discipline to delay spread and angular
...
the frozen Try 78 path-loss prior and Try 79 spread priors, then learns
...
\chapter{Final Residual GMM-Head Model Configuration Summary}

The final residual GMM-head model configuration uses the canonical CKM dataset at image
size \(513\times513\), split seed 42, and validation/test ratios of 15\% each
after city assignment. It uses
precomputed priors when available through a saved prior-cache file, but the
priors are deterministic and can also be recomputed.

\begin{table}[H]
\centering
\caption{Main final residual GMM-head model configuration values.}
...
Category & Value \\
\midrule
Input channels & 9 aligned maps: topology; LoS/NLoS/ground masks; combined,
LoS and NLoS path-loss priors; delay-spread prior; angular-spread prior. \\
Height conditioning & Sinusoidal embedding with FiLM modulation. \\
Backbone & Four-scale GroupNorm encoder-decoder, base width 128, condition
dimension 160. \\
Residual distribution & Three mixture components per task/region head. \\
Residual limits & PL: 7 dB LoS, 25 dB NLoS; DS: 20 ns LoS/NLoS; AS:
\(17^\circ\) LoS, \(15^\circ\) NLoS. \\
Pre-fine-tune losses & Large-model stage
\texttt{try80\_joint\_huge\_relaxed\_residuals}: NLL 1.0, KL 0.25,
moments 0.1, anchor 0.05, prior-guard 0.1, RMSE 1.5, MAE 0.1, outlier
budget 0.02. \\
Final fine-tune losses & Selected stage
\texttt{try80\_joint\_huge\_pathloss\_finetune}: RMSE 1.0, MAE 0.05,
anchor 0.01, prior-guard 0.02. NLL, KL, moments and outlier terms remain
implemented but have zero final weight. \\
Training & AdamW, \(5\times10^{-5}\) learning rate, 0.01 weight decay, 40
fine-tune epochs, batch size 1, accumulation 8, cosine \texttt{LambdaLR} after
one warmup epoch. \\
\bottomrule
\end{tabularx}
\endgroup
...
The non-fine-tuned large-model weights are listed separately because the final
fine-tune changes the optimization objective while keeping the same large
architecture and relaxed residual caps.
...
inference, and the frozen calibrated priors. The script selects one validation
...
\caption{Final residual GMM-head model \(6\times3\) inspection panel for an open, high-antenna
...
only the parts most relevant for reproducing the prior-anchored final residual GMM-head model design.
...
The final prior computer combines the calibrated path-loss prior with
the calibrated spread priors. The path-loss branch keeps separate LoS
...
delay_prior = _compute_try79_metric_prior(
    metric="delay_spread", topology_class=ct6, ant_label=ant,
    los_mask=los_mask, shared=shared, coefs=self._spread_cal)
angular_prior = _compute_try79_metric_prior(
    metric="angular_spread", topology_class=ct6, ant_label=ant,
    los_mask=los_mask, shared=shared, coefs=self._spread_cal)
\end{verbatim}
}

\section{Final residual GMM-head model input tensor}
...
\setlength\itemsep{0.15em}
  ...
  \item \texttt{Run final model}: disables neural inference when only masks
...
The final benchmark was repeated on the same ten Barcelona configurations used
for the MATLAB ray-tracing check: two layouts, five UAV heights per layout, and
a \(513\times513\) full-resolution map. The timed final-generator path starts
after the checkpoint has been loaded once and includes geometric LoS/NLoS mask
generation, frozen path-loss and spread-prior computation, and neural
prediction of path loss, delay spread, and angular spread. On the
RTX~3050~Ti Laptop GPU, CUDA
mixed precision with batch size 1, the vectorized CUDA LoS/NLoS backend, and
the torch prior backend took \SI{0.88}{s} per map end to end. The component
means were \SI{0.30}{s} for LoS/NLoS mask generation, \SI{0.04}{s} for the
frozen priors, and \SI{0.54}{s} for the final residual GMM-head neural forward
pass. The
mask-generation stage was the previous bottleneck; after vectorization and
torch prior evaluation, the network forward pass is the largest single
component.

The vectorized mask backend was validated as an implementation optimization,
not as a change in mask definition. On the ten Barcelona configurations and on
100 exported CKM samples, it produced zero LoS/NLoS differences with the
original GT ray-caster; the optimization only changes how the same visibility
test is evaluated. The torch prior backend was validated separately on the full
50-sample Barcelona city subset with stored targets. Relative to the NumPy
prior backend, full-precision RMSE changed only in the fourth decimal place:
\SI{1.7417}{dB}, \SI{32.9652}{ns}, and \SI{11.0164}{\degree} for path loss,
delay spread, and angular spread.

For comparison, the MATLAB ray-tracing script took \SI{99.89}{s} per
full-resolution Barcelona sample on the same laptop. The local MATLAB
installation did not include Parallel Computing Toolbox, so the ray tracer ran
on CPU, while the surrogate used the available CUDA runtime through PyTorch.
The resulting measured speed-up is \(113.7\times\) for the complete non-RT
generation path, including priors.
...
\texttt{Run final model} is useful when the goal is only to validate
