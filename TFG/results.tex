...
which are reproducible and evaluated on the final city-holdout test split.
...
city-holdout test set (pixel-weighted RMSE).}
...
\textbf{Coherent two-ray}       & \textbf{1.7516} & $\mathbf{-1.96}$ \\
\bottomrule
\end{tabular}
\end{table}
...
switching to a \emph{coherent} two-ray model with a fitted effective reflection
...
result because it is obtained without injecting any physics. Full derivation
and per-expert numbers are treated in the methodology chapter and in the
Try 76 appendix material.
...
angular spread. It partitions the dataset into the same six topology classes
and trains two GMM experts per class (one per spread target), for a total of
twelve experts. The training code, per-expert configuration files, and
submission scripts are all in the repository
(\texttt{TFGpractice/TFGSeventySeventhTry77/}), but the evaluation artefacts
needed for a per-expert table are produced on the SLURM cluster and have not
been mirrored into this document yet. Try 77 is therefore included in the
methodology chapter as the architecture that produced the GMM spread priors
consumed indirectly through Try 79, with final per-expert numbers deferred to
the appendix once the cluster outputs are pulled.
...
the city-holdout test split.}
\label{tab:try78_by_height}
\begin{tabular}{lcccc}
\toprule
UAV height bin (m) & Overall RMSE (dB) & LoS RMSE (dB) & NLoS RMSE (dB) & \# samples \\
\midrule
...
\begin{tabular}{lcccc}
\toprule
UAV height bin (m) & Metric & Overall & LoS & NLoS \\
\midrule
...
each row is one of the three targets (path loss, delay spread, angular
spread) and the six columns are, from left to right, the ground truth, the
frozen Try 78/79 prior, the Try 80 prediction, a context or support map,
the prior error \(\mathrm{Prior}-\mathrm{GT}\), and the model correction
\(\mathrm{Try\,80}-\mathrm{Prior}\). The last two columns are the main
diagnostic: if the residual head is doing its job, the sign and location of
the correction (column~6) should mirror the sign and location of the prior
error (column~5).

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth,height=0.86\textheight,keepaspectratio]{img/thesis_figures/try80_appendix_panels/try80_main_panel.png}
\caption{Representative Try 80 inspection panel for a validation sample.
Rows: path loss, delay spread, angular spread. Columns: ground truth,
frozen prior (Try 78/79), Try 80 prediction, context / support mask, prior
error ($\mathrm{Prior}-\mathrm{GT}$), and model correction
...
Delay spread & \SI{1}{ns} & \SI{0.289}{ns} & \(\approx\SI{0.036}{ns}\) \\
