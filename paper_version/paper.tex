...
RMSE-oriented fine-tuning; reported maps use the pixel-wise mixture expectation.
...
around the frozen priors and then fine-tunes the deterministic expectation. \\
\bottomrule
\end{tabularx}
\end{table}
...
is the usual mobile-height correction. The second term is an A2G
...
\(\rho(h_{\mathrm{tx}}),\phi(h_{\mathrm{tx}})\) & LoS PL & Train-split fit & Effective reflection amplitude and phase; not material constants. \\
\(b(h_{\mathrm{tx}})\) & LoS PL & Train-split fit & Height-dependent offset left after coherent two-ray fitting. \\
\(r(d_{2D},h_{\mathrm{tx}})\) & LoS PL & Train-split fit & Smoothed radial residual left by two-ray plus bias. \\
\bottomrule
\end{tabularx}
\end{table*}
...
training pixels. For each candidate pair, the dB bias has the closed-form value
\begin{equation}
...
\end{equation}
with fallback to broader topology/height/visibility groups when the exact
regime has too few pixels. At deployment, the same deterministic features select
one stored coefficient vector and produce the clipped NLoS prior in
Eq.~\eqref{eq:nlos-prior-paper}.
...
summarized in Table~\ref{tab:prior-only-anchor}. The final neural model
receives only the resulting prior maps as channels; the handcrafted features
remain inside the frozen prior estimators rather than becoming additional
learned inputs.
...
\(17^\circ/15^\circ\) for AS. Omitting indices for compactness, the component
mean is anchored to the frozen prior in the same transformed space used by the
implementation:
\begin{equation}
\begin{aligned}
u&=y^{\mathrm{prior}}+\alpha\Delta,\\
\mu&=u,\qquad k=\mathrm{PL},\\
\mu&=\log(1+[u]_+),\qquad k\in\{\mathrm{DS},\mathrm{AS}\}.
\end{aligned}
\label{eq:mu-paper}
\end{equation}
where \([u]_+=\max(u,0)\) and \(0\leq\alpha_{k,r}(p)\leq1\). PL is modeled in
native dB space, while DS and AS use the log transform in
...
far the prediction may move from the frozen estimate. The final expectation is
therefore not an unconstrained average of arbitrary map values; it is the mean
of a small residual mixture whose components remain tied to the prior.
...
\node[mix] (mu) at (12.3,0.7) {Anchored means\\PL: \(y^{\mathrm{prior}}+\alpha\Delta\)\\DS/AS: \(\log(1+[y^{\mathrm{prior}}+\alpha\Delta]_+)\)};
...
Final PL is numerically lower; recent corridor-weighted outdoor PL predictor, but no variable-height UAV or joint spread outputs. \\
...
same band as strong dense radio-map references and numerically below the recent
Gao et al.\ outdoor segmentation results, while the spread results should be
interpreted primarily against same-protocol internal baselines because no
