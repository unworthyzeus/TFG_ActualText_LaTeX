...
maps with a probabilistic residual head. The model predicts \(513\times513\) dense channel attenuation,
DS, and AS maps from
...
UAV simulation contract: strict city holdout, valid receiver masking, explicit
...
The head uses a two stage residual logic around the frozen prior. The global stage asks:
...
\end{equation}
with both terms constrained to \([0.05,3.0]\). During the probabilistic residual stage
...
the probabilistic residual stage the per pixel density is
\begin{equation}
q_{k,r}(z_k(p)\mid p)
=
\sum_{j=1}^{3}
...
training phases: a large capacity probabilistic residual stage with probabilistic losses
...
The probabilistic terms matter during the probabilistic residual stage. The NLL term
...
Moment matching & probabilistic residual stage &
...
Outlier budget & probabilistic residual stage &
...
Loss schedule & Large capacity probabilistic residual stage plus final residual
...
\begin{table}[t]
\centering
\caption{\textsc{HARP-Net CKM} loss schedule. The pre fine tuning probabilistic residual stage is part
