...
coherent two-ray map of Eq.~\eqref{eq:pl_los}.
...
calibrated coherent two-ray prior, so Eq.~\eqref{eq:pl0} and
...
because LoS pixels are replaced by the separate calibrated coherent two-ray
prior.
...
Coherent two-ray (LoS)     & 1.7516 & \textbf{frozen LoS prior} (used in Try 80)\\
Hybrid LoS                 & 1.7516 & legacy auxiliary variant (not used in final Try 80 path)\\
...
This comparison is included for historical completeness. In the current
\texttt{Try 80} implementation, the frozen LoS prior is the calibrated
coherent two-ray map; the Hybrid LoS blend is not used in the final path-loss
assembly. The two rows report the same value (1.7516 dB) on the
\emph{evaluation split} because, on almost all pixels, the auxiliary A2G
envelope does not change the underlying LoS value.
