...
are kept in Appendix~\ref{app:method_details}, so the methodology chapter can
stay short while the full audit trail remains available.
...
\end{tikzpicture}
\caption[\textsc{HARP-Net CKM} pipeline with bounded DNN corrections.]
{\textsc{HARP-Net CKM} pipeline. The frozen priors are injected into the DNN, which learns a bounded correction $\delta$ around them.}
\label{fig:pipeline}
\end{figure}
...
the distribution aware head represents residual uncertainty without turning the
main method into an unconstrained direct regressor.
...
detailed equations and fitting procedures are given in
Appendix~\ref{app:method_details}, covering the attenuation prior, spread
prior, and \textsc{HARP-Net CKM} residual model.
