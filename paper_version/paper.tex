...
\textsc{HARP-Net CKM}, a hybrid surrogate that freezes physical and statistical
priors calibrated on the training split and learns bounded residual maps with a Gaussian
mixture head. The model predicts \(513\times513\) dense channel attenuation,
DS, and AS maps from
topology, geometric masks, UAV transmitter height, and frozen priors through a
...
the same contract: continuous UAV transmitter height, strict city holdout on CKM samples,
...
but the model contract is an elevated \SI{7.125}{GHz} transmitter within the
...
transmitter height, topology derived morphology, and the geometric LoS/NLoS mask. Once
...
transmitter height conditioning path because the framework has already computed the
...
\SI{5}{m} transmitter height bins to reproduce the ring like constructive and
...
is directly valid at every transmitter height:
\begin{equation}
\begin{aligned}
\mathrm{PL}_{C}
...
\label{eq:x79-paper}
\end{equation}
Here \(h\) is normalized transmitter height, \(b_{41}\) is a local blocker depth
...
the transmitter altitude range. This is a stricter check than the global row because it
...
A retained distribution diagnostic changed the modeling strategy: LoS and NLoS
channel attenuation do not form a single easy Gaussian map target. The
...
the held out test set. Continuous transmitter height is handled through both the
...
accurate, fast, and interpretable enough to reduce repeated ray traced map
generation for many planning experiments, achieving a measured \(113.7\times\)
speedup over the local MATLAB ray tracing path. The result is a reproducible surrogate for a
