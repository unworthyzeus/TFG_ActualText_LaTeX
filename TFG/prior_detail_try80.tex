...
The final \texttt{Try 80} checkpoint reported in Chapter~\ref{chap:results}
uses these frozen \texttt{Try 78}/\texttt{Try 79} priors as anchors. This
...
The numerical weights used by the final
\texttt{try80\_joint\_huge\_pathloss\_finetune} checkpoint are
\(\lambda_{\mathrm{NLL}}=0\), \(\lambda_{\mathrm{KL}}=0\),
\(\lambda_{\mathrm{mom}}=0\), \(\lambda_{\mathrm{anchor}}=0.01\),
\(\lambda_{\mathrm{guard}}=0.02\), \(\lambda_{\mathrm{out}}=0\),
\(\lambda_{\mathrm{RMSE}}=1.0\), and \(\lambda_{\mathrm{MAE}}=0.05\).
The task weights are \(w_{\mathrm{PL}}=1.0\),
\(w_{\mathrm{DS}}=0.25\), and \(w_{\mathrm{AS}}=0.25\). The validation
checkpoint score uses separate reporting weights
\((1.0,\,0.30,\,0.25)\) for path loss, delay spread and angular spread,
respectively.
...
\caption[Composite \texttt{Try 80} loss family]{Composite \texttt{Try 80}
