...
released code keeps this term for consistency with the stored calibration
vector. However, in the final code path, deleting this feature or setting only
this coefficient to zero changes the resulting path loss maps by less than
\SI{1}{dB}, so the quadratic term has little practical influence compared with
the linear raw prior, morphology, elevation and bias terms.
...
physically unreasonable values in the obstructed branch. This clipping is not a
source of measured accuracy gain: the clipped and unclipped versions give the
same results on the training, validation and test sets. It is kept for
consistency with the dataset path loss range and as a conservative guard
against possible outliers in very unusual environments not represented in the
available CKM cities.
