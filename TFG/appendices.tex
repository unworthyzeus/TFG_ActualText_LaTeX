...
\texttt{best\_model.pt} checkpoint, and the frozen Try 78/79 priors. The
script selects one validation sample for each macro expert
\((\mathrm{open},\mathrm{mixed},\mathrm{dense})\times
(\mathrm{low},\mathrm{mid},\mathrm{high})\), and --- because the purpose of
the appendix is to show the prior-anchored story in action --- only keeps
candidates for which the Try 80 prediction beats the frozen prior on all
three outputs simultaneously (path loss, delay spread, angular spread). Each
bucket then picks the candidate with the best mean model RMSE. A JSON
manifest next to the images records the file names, cities, samples, antenna
height, and per-task prior and model RMSE.

The script emits each panel in two equivalent layouts, so the appendix can use
whichever one fits the page orientation better. The \textbf{wide} layout
(\(3\times 6\)) uses the three targets as rows and the six inspection
stages as columns; the \textbf{tall} layout (\(6\times 3\)) transposes that
into six inspection-stage rows and three target columns. The six inspection
stages, in order, are:
...
\begin{enumerate}
  ...
  \item \textbf{Model correction} --- the signed residual
        ...
        column~6 should \emph{anti-correlate} with column~5: where the prior
        is too high, the model should push down, and vice versa.
\end{enumerate}

Columns 5 and 6 are therefore the main \emph{diagnostic} columns: the thesis
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{img/thesis_figures/try80_appendix_panels/try80_panel_open_low_ant_Chiang_Mai_sample_04525.png}
\caption{Try 80 appendix panel for an open, low-antenna validation sample.}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{img/thesis_figures/try80_appendix_panels/try80_panel_open_mid_ant_Chiang_Mai_sample_04521.png}
\caption{Try 80 appendix panel for an open, mid-antenna validation sample.}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{img/thesis_figures/try80_appendix_panels/try80_panel_mixed_low_ant_Chiang_Mai_sample_04438.png}
\caption{Try 80 appendix panel for a mixed, low-antenna validation sample.}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{img/thesis_figures/try80_appendix_panels/try80_panel_mixed_mid_ant_Chiang_Mai_sample_04436.png}
\caption{Try 80 appendix panel for a mixed, mid-antenna validation sample.}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{img/thesis_figures/try80_appendix_panels/try80_panel_dense_low_ant_Chiang_Mai_sample_04446.png}
\caption{Try 80 appendix panel for a dense, low-antenna validation sample.}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{img/thesis_figures/try80_appendix_panels/try80_panel_dense_mid_ant_Chiang_Mai_sample_04448.png}
\caption{Try 80 appendix panel for a dense, mid-antenna validation sample.}
