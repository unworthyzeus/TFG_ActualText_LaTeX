...
The final model assigns stable large scale structure to frozen priors and
reserves the neural network for bounded residual correction:
...
The priors keep the deterministic or interpretable structure, while the deep
model handles the residual part that the prior cannot explain.
Figure~\ref{fig:methodology_full_framework} illustrates the overall pipeline.

The prior phase has a practical role: it turns physics, morphology, visibility,
and height dependent calibration into a strong auditable baseline. The final
model then follows a split of responsibility: calibrated priors remove stable
target structure, and the neural residual model focuses on the local errors that
remain.

In this chapter, a prior is a deterministic frozen estimator map computed before
the neural residual correction. Three such estimators are used:
\begin{itemize}
\setlength{\itemsep}{0.16em}
\setlength{\parsep}{0pt}
\setlength{\topsep}{0.15em}
\item \textbf{LoS path loss prior.} This is the clearest physical branch:
      \(\mathrm{PL}_{\mathrm{LoS}}=\mathrm{FSPL}+C_{\mathrm{2ray}}+\beta_{\mathrm{LoS}}+r\).
      FSPL uses the 3D direct Tx--Rx distance; \(C_{\mathrm{2ray}}\) comes
      from a coherent direct plus ground reflected field sum
      \cite{rappaport,wocc2021}; \(\beta_{\mathrm{LoS}}\) is a fitted height
      dependent dB offset; and \(r\) is a smoothed radial
      ...
      \(\rho(h_{\mathrm{tx}})\), \(\phi(h_{\mathrm{tx}})\),
...
\item \textbf{Delay and angular spread priors.} These are fitted in
      ...
      adds the 23-feature vector of Sec.~\ref{sec:spread_prior_detail}. That vector
...
The notation is kept distinct across blocks. The symbol
\(\rho(h_{\mathrm{tx}})\) is reserved for the LoS reflection amplitude.
Local building density is written as \(\delta_k\), and the LoS dB bias is
written as \(\beta_{\mathrm{LoS}}(h_{\mathrm{tx}})\). The spread symbols
\(b_{\mathrm{topology}}\) and \(b_{41}\) remain local to the spread prior:
the first is a log domain topology offset and the second is a blocker depth
feature.
...
Height interpolated effective reflection phase, in radians. It absorbs the
phase offset of the simplified one reflection geometry. \\
\(\beta_{\mathrm{LoS}}(h_{\mathrm{tx}})\) & LoS path loss &
...
A useful prior is a complete estimator: the formula, fitted calibration,
feature units, LoS/NLoS mask, topology features, and evaluation mask must match
between fitting and inference. The final prior is strong because it keeps those
pieces self contained and auditable under the same city holdout split used for
the final model.
...
The same constraint motivates the residual and distribution aware network.
...
\textsc{HARP-Net CKM} keeps the priors as frozen anchors and lets the neural
head model bounded residual structure and uncertainty within those anchors.
