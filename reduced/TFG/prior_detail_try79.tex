...
\begin{tikzpicture}[
    ...
    block/.style={draw, thick, fill=blue!10, rectangle, rounded corners,
      text width=2.55cm, minimum height=1.0cm, align=center},
...
\node[input] (inputs) at (0,0) {Geometry, masks and morphology\\
$d_{2D}, \theta, h_{\mathrm{tx}}, \rho, h, n$\\$b_{41}, c_{41}, t_{41}$};
\node[block] (raw_prior) at (3.2,0) {Stage 1\\Raw log prior\\$z^{(0)}(x)$};
\node[block] (features) at (6.4,0) {Stage 2\\Feature vector\\$\textbf{x}_{79}(x)\in\mathbb{R}^{23}$};
\node[block] (ridge) at (9.6,0) {Stage 3\\Regime ridge\\$\hat z=\textbf{x}_{79}^{\top}\hat{\textbf{w}}^{79}_{r}$};
\node[branch] (invert) at (12.8,0) {Stage 4\\Inverse transform\\$\hat y=\exp(\hat z)-1$};
\node[input] (clip) at (16.0,0) {Calibrated spread prior\\$\hat y(x)$ after clipping};
\node[note] (keys) at (9.6,-1.75) {Regime \(r\) keys:\\metric (DS/AS), topology, LoS/NLoS, antenna bin};
\draw[arr] (inputs) -- node[above, font=\tiny] {maps} (raw_prior);
\draw[arr] (raw_prior) -- node[above, font=\tiny] {$\mathbb{R}^{H\times W}$} (features);
\draw[arr] (features) -- node[above, font=\tiny] {$\mathbb{R}^{23\times H\times W}$} (ridge);
\draw[arr] (ridge) -- node[above, font=\tiny] {$\mathbb{R}^{H\times W}$} (invert);
\draw[arr] (invert) -- node[above, font=\tiny] {$\mathbb{R}^{H\times W}$} (clip);
\draw[arr, dashed] (keys) -- (ridge);
\draw[arr, dashed] (inputs.south) .. controls +(1.4,-1.2) and +(-1.4,-1.2) .. (features.south);
\end{tikzpicture}%
}
