...
post-smoothing; and ridge regression with $\lambda = 10^{-3}$ over 15 (path
loss) and 23 (spread) morphology/geometry features per regime for the NLoS and
spread branches. Therefore, the novelty is not ``inventing a new textbook
propagation law'', but building a practical hybrid pipeline that is both
interpretable and trainable on the dataset at hand.
\end{quote}
