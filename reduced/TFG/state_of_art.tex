...
Chapter~\ref{chap:results} as scale references for the final path-loss result.
...
the residual prediction, which penalises locally non-smooth outputs without
...
Chapter~3 applies this philosophy to the three thesis targets. For path loss,
an enhanced two-ray model fitted per height bin provides a prior map
...
multi-modality, leaving a compact mixture to represent under correction,
...
path loss, delay spread, and angular spread. The literature point is that joint
...
\subsubsection{Gao et al.: multi-layer segmentation with corridor weighting}
