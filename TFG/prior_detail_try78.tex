...
The coherent two-ray model is a staple of wireless propagation theory, but our
implementation in \texttt{Try 78} deviates from the standard textbook
...
In contrast, our model (Eq.~\ref{eq:field_sum}) replaces the angle-dependent
...
and $\phi$ are fitted to the CKM dataset. This allows the model to capture
...
grazing angles. Our model uses the \emph{exact} field sum (Eq.~\ref{eq:field_sum})
without the $d^4$ approximation, which is necessary because UAV heights are
often significant relative to horizontal distances, making the small-angle
approximation invalid.

\paragraph{Over-unity amplitude ($\rho > 1$).}
Standard physics dictates $|\Gamma| \le 1$. However, our calibration grid
search (Sec.~\ref{sec:los_calibration}) allows $\rho \in [0, 1.5]$. While
physically impossible for a single specular reflection, an effective $\rho > 1$
allows the linear basis of the two-ray model to capture constructive multipath
interference from multiple scatterers (e.g., building faces near the transmitter)
that align with the geometric phase of the ground ray.

\paragraph{Radial correction.}
Standard models \cite{tr38901} treat the two-ray or FSPL result as the final
mean. Our model adds a calibrated radial residual $r(d_{2D}, h_{\mathrm{tx}})$
(Eq.~\ref{eq:pl_los}) which acts as a data-driven "mopping up" term for
systematic geometric errors that neither FSPL nor the two-ray model can
fully represent.
...
optimistic at constructive-interference peaks in the LoS case.
