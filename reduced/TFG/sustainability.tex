...
Faster elevated base station or UAV coverage planning for temporary or
emergency networks. &
...
work was electricity for computation. The \textsc{HARP-Net CKM} line used four RTX~2080
...
\SI{27}{kWh}. The ML-calibrated priors are lighter than the \textsc{HARP-Net CKM} residual head
...
the number of repeated ray-tracing simulations required during planning. A ray tracer is
...
rather than in every planning query. For the simulated FR3 elevated transmitter
setting studied in this thesis, centred on \SI{7.125}{GHz}, the surrogate can
generalise across the held out cities without launching a new ray-tracing run
for each antenna height or urban map.
The surrogate can also be used for fast what-if analyses during planning:
it should not be treated as new reference data, but it can evaluate
many elevated transmitter heights or urban alternatives at much lower marginal
energy cost.
The runtime measurements in Chapter~\ref{chap:results} make this concrete for
the present dataset scale. On the Barcelona benchmark, pixel level MATLAB ray
tracing took \SI{99.89}{s} per full-resolution CKM sample, while the complete
\textsc{HARP-Net CKM} generator took \SI{0.88}{s} per sample under the same
benchmark contract.
Extrapolated to 16{,}180 samples, this is \SI{448.96}{h} for ray tracing versus
\SI{3.95}{h} for the surrogate. Under the simple same-laptop full-load
assumption of \SI{100}{W}, the energy figures are approximately
\SI{44.9}{kWh} and \SI{0.39}{kWh}, respectively. These figures are not a full
life-cycle assessment, but they show the main sustainability advantage of the
approach: once the model is trained, repeated CKM generation is about
\(113.7\times\) faster and, under the same power-envelope assumption, about
\(113.7\times\) more energy efficient.
...
Supervision meetings and supervisors' time for dataset generation from simulation & 50\,h         & 40\,EUR/h        & 2\,000\,EUR & Advisor time \\
GPU compute, \textsc{HARP-Net CKM}    & 48\,GPU-h     & 0.25\,EUR/GPU-h  &    12\,EUR  & 4$\times$ RTX\,2080\,Ti, 12\,h \\
...
Electricity                        & N/A           & incl.\ in GPU-h  &     0\,EUR  & Covered by GPU rental rate \\
Storage and code hosting           & N/A           & free             &     0\,EUR  & GitHub \\
\midrule
...
under the same \SI{7.125}{GHz} CKM contract.
...
performance without field measurements. The \textsc{HARP-Net CKM} results are reported
from a single selected model state on the city holdout protocol, so the relevant
