...
dense UAV CKM generation remains difficult when channel attenuation, delay spread, and
angular spread must all generalize to unseen cities. This paper presents
\textsc{HARP-Net CKM}, a hybrid surrogate that freezes train only calibrated
physical/statistical priors and learns bounded residual maps with a Gaussian
mixture head. The model predicts \(513\times513\) dense channel attenuation,
DS, and AS maps from
topology, geometric masks, UAV height, and frozen priors through a
probabilistic residual stage followed by deterministic RMSE oriented fine
tuning. On held out cities it reaches \SI{1.6519}{dB} channel attenuation RMSE,
\SI{26.5570}{ns} delay spread RMSE, and \(11.3854^\circ\) angular spread RMSE,
reducing RMSE relative to the frozen priors for all three targets. The result
supports a hybrid design principle: freeze stable radial, visibility, height,
and spread structure in auditable priors, and reserve neural capacity for the
residual distribution those priors cannot explain.
\end{abstract}
...
errors remained large under a ground only metric. PMNet inspired and U-Net
style backbones improved smooth LoS structure but did not solve NLoS mode
collapse or the spike plus tail nature of spread targets. The final design
therefore moved away from ``bigger backbone first'' and toward a stricter
sequence: define the evaluation contract, diagnose target distributions,
calibrate train only priors, and then train a bounded residual model around
those frozen anchors. The contribution is \textsc{HARP-Net CKM}, a dense CKM
recipe for one fixed UAV simulation contract: strict city holdout, valid
receiver masking, explicit LoS/NLoS and height reporting, and a final prior
anchored neural correction.
...
the same contract: continuous UAV height, strict city holdout on CKM samples,
...
clouds \cite{mi2024pointcloud}. These works are close in target type but still
predict link level or sparse channel parameters, not dense \(513\times513\)
CKM maps for unseen cities.
...
loss MAE is \SI{0.9298}{dB}, close to the label step, while RMSE remains
\SI{1.6519}{dB} because it emphasizes hard tails. Less quantized labels would
...
Second, NLoS channel attenuation is not just a smooth residual; it is a blocked link
...
Earlier GMM heads without priors &
Distribution diagnostic &
Worse than the final system, but showed that channel attenuation, DS, and AS are jointly
predictable when the head represents LoS/NLoS and heavy tailed targets. \\
Frozen attenuation and spread priors &
Structured anchor &
...
\end{tikzpicture}}
\caption{Compact prior anchored pipeline used for the reported final system.
...
prior anchored residual GMM model.}
\label{fig:pipeline}
\end{figure*}
...
UAV height conditioning path because the framework has already computed the
visibility mask, receiver mask, and frozen prior maps.
...
fit. The direct distance \(d_{\mathrm{LoS}}\) sets the free space baseline,
\(d_{\mathrm{ref}}\) supplies the image source ground reflected path, and
\(k(d_{\mathrm{ref}}-d_{\mathrm{LoS}})\) is the reflected wave phase lag.
The calibrated \(\rho(h_{\mathrm{tx}})\), \(\phi(h_{\mathrm{tx}})\),
\(b(h_{\mathrm{tx}})\), and \(r(d_{2D},h_{\mathrm{tx}})\) curves are fitted in
\SI{5}{m} height bins, smoothed, interpolated at inference, and guarded to the
\([20,180]\) dB range. The notation is local: \(\rho(h_{\mathrm{tx}})\) is the
LoS reflection amplitude, whereas \(\rho_k\) denotes local building density;
similarly \(b(h_{\mathrm{tx}})\) is a LoS bias, while \(b_q^{\mathrm{nat}}\),
\(b_{\mathrm{top}}\), and \(b_{41}\) belong to the spread prior.
...
destructive interference visible in LoS channel attenuation maps. Second, the per height
...
This explicit LoS branch is diagnostic: many CKM LoS errors are radial rings
around the centred transmitter rather than building edge artifacts. The
two ray plus radial table removes that height/radius component before the
network spends capacity on local visibility and morphology corrections.
...
to \SI{7.125}{GHz} UAV links. The second term is a conservative A2G NLoS
envelope: it keeps altitude/elevation structure from coverage models, then lets
the train only calibration correct the fixed excess loss constants:
\begin{equation}
\begin{aligned}
...
\end{equation}
The maximum is deliberately pessimistic: whichever raw branch predicts stronger
attenuation is kept. It is then corrected by train only regime wise linear
calibration:
\begin{equation}
...
The NLoS side adds calibration terms for height, building topology, and
density. This feature choice follows the same modelling instinct as COST 231
Walfisch Ikegami: NLoS urban loss should depend on the surrounding built form,
not only on range and frequency \cite{cost231}. The Walfisch Bertoni component
treats rows of buildings as repeated diffraction screens \cite{walfisch1988}.
The Ikegami street level component motivates descriptors such as building
height, street width, street orientation, and mobile height \cite{ikegami1984}.
In the CKM maps these coarse planning variables are replaced by directly
observed local windows: building density, mean building height, and NLoS
support fractions. A modern machine learning analogue is Juang's path profile
work, where building profile statistics are used in a linear model to explain
urban path loss predictions \cite{juang2021pathprofile}. The raw blocked link
prior is therefore only one feature in a 15 term morphology vector:
\begin{equation}
\begin{aligned}
\mathbf{x}_{78}=[&
...
\SI{5}{m} height bins &
LoS channel attenuation map carrying the dominant height/radius rings before any neural
residual is added. \\
PL NLoS &
...
NLoS channel attenuation map that stores blocked link scale, local density, blocker, and
elevation effects. \\
DS / AS &
...
above ground building height \(\bar h_{\mathrm{map}}\): coverage
...
All calibrations use training cities only. The spread prior dictionary contains
114 fitted regime keys: \(36\) exact topology/visibility/height keys per
metric plus topology, visibility, and global fallbacks. Height is handled
differently across priors: the LoS PL branch interpolates \SI{5}{m} curves,
whereas NLoS PL and spread branches use antenna height regimes
(\texttt{low\_ant}, \texttt{mid\_ant}, \texttt{high\_ant}) while still keeping
continuous height, clearance, and taller building features inside
\(\mathbf{x}_{79}\). The point of this heavy calibration is to store many small
decisions about morphology, visibility, and height before the neural residual
is trained. On the final held out test split, the deployed prior anchors are
summarized in Table~\ref{tab:prior-only-anchor}.
...
\begin{table}[t]
\centering
\caption{Final prior anchored loss schedule. The pre fine tuning residual/GMM stage is part
...
model reduces RMSE relative to the frozen prior for all three targets. Channel attenuation
...
the same 10840/2750/2590 train/validation/test contract.
...
LoS and NLoS channel attenuation do not form a single easy Gaussian map target. The
...
claims. The earlier distribution first channel attenuation GMM fits reached \SI{4.43}{dB} LoS
...
\caption{Full non residual DL GMM head model for channel attenuation (PL/\texttt{path\_loss}): target GMM-5
...
Final attenuation result is in this band; no UAV height or spread targets. \\
...
Final attenuation result is close but higher under a different benchmark contract; scale context only, no joint spread prediction. \\
...
\textbf{\SI{1.6519}{dB} attenuation; \SI{26.5570}{ns} DS; \(11.3854^\circ\) AS} &
...
\textbf{Dense channel attenuation/DS/AS maps under the final city holdout contract.} &
...
The fair conclusion is narrow but useful. The final channel attenuation number is in the
...
same protocol internal baselines because no external dense CKM channel attenuation/DS/AS
benchmark was found.
...
correlate negatively with the prior error row, which is the expected behavior of a
...
model outputs continuous channel attenuation, DS, and AS values. It is a limitation of the stored
labels. For channel attenuation, a \SI{0.9298}{dB} final MAE against labels quantized at
...
metrics. The reported tables are tied to the selected \texttt{best\_model.pt}
checkpoint and its exported comparison summary and deep breakdown JSON files,
rather than to a later interactive demo run. For traceability, the broader development history
...
predicts channel attenuation, delay spread, and angular spread on unseen cities. Under the
...
channel attenuation residuals are mixture-like and prone to mode collapse, while DS/AS
