...
\par\smallskip
\noindent\textbf{How $\textbf{w}^{\mathrm{PL,NLoS}}_{r}$ is fitted.}
\par\smallskip
\noindent Across all training samples whose regime label equals $r$, a linear
regression is run:
...
This term was originally included to test whether the NLoS calibration needed a
quadratic relationship with the raw path loss prior. Its coefficient is visibly
much smaller than most other entries, and tests showed that removing this
feature or setting only this coefficient to zero does not change the maps in
any practical way. It is kept only because the stored final calibration vector
was very expensive to rerun: the calibration processes roughly \(10\,000\)
\(513\times513\) pixel images, even though only valid pixels enter the fit. The
