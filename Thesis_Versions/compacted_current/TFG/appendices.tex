...
chronology is separated from the detailed final formulas: Chapter~3 contains
the compact methodology, Chapter~\ref{chap:results} contains the quantitative
results, and Appendix~\ref{app:formula_details} preserves the full mathematical
audit trail.
...
\chapter{Additional Qualitative Diagnostics}
...
\chapter{Quantisation Detail for the Target Maps}
\label{app:quantisation_detail}

The path-loss target is stored as uint8, delay spread as uint16 in ns, and
angular spread as uint8 in degrees. The effect can be written explicitly. Let
\(y^\star\) be the continuous ray-traced value and
\(y_q = Q_\Delta(y^\star) = y^\star + \epsilon_q\) the stored value after
rounding to step \(\Delta\). Under the usual high-resolution quantisation
approximation, \(\epsilon_q \sim \mathcal{U}(-\Delta/2,\Delta/2)\), so
\begin{equation}
...
and \(0.289^\circ\) for angular spread.

\begin{table}[H]
\centering
\caption[Approximate RMSE floor from target quantisation.]%
{Approximate quantisation RMSE implied by the stored integer target precision
in the current CKM HDF5 file.}
...
If model error and quantisation error are approximately independent, the
observed squared error decomposes as
\[
...
\chapter{Detailed Final Model Formula Documentation}
\label{app:formula_details}

This appendix keeps the full mathematical and fitting detail for the final
path-loss prior, spread priors, and joint residual GMM-head model. The main
methodology chapter gives the decision path and compact model definition; this
appendix preserves the derivations, feature inventories, calibration notes, and
loss details for auditability.

\section{Detailed Path-Loss Prior}
\input{prior_detail_try78}

\section{Detailed Delay and Angular Spread Priors}
\input{prior_detail_try79}

\section{Detailed Joint Prior-Anchored Residual GMM-Head Model}
