...
the central results of the thesis: a strong baseline calibrated only on
training cities that absorbs the stable part of the map before the residual
model is asked to solve the harder local errors. This chapter focuses on the
frozen priors and final test evaluation.
...
Under this evaluation protocol, the \SI{1.6950}{dB} attenuation RMSE is a very
strong result. It is not directly comparable with easier radio map challenge
scores that use fixed transmitter heights, random splits, different clipping
or different building masks, but it is well below the original \SI{5}{dB}
target and close to the quantization scale of the stored dB maps. The spread
numbers should be read differently: the reviewed literature rarely reports
dense per pixel delay spread or angular spread maps over unseen cities with
the same inputs. Their strongest interpretation is therefore internal and
same protocol: both targets beat the project thresholds and both improve over
the frozen priors.
...
channel characteristics or use different simulation setups, so the main
...
here, \SI{1.6950}{dB} is a strong result for unseen cities, continuous UAV
