...
computed by the regime wise path loss calibration, not a map that has been
...
path loss prior channels are divided by \(180\,\mathrm{dB}\); and the
delay and angular spread priors are passed as
\(\log(1+\mathrm{prior})/\log(1+400)\) and
...
reported metrics remain in the native units.
