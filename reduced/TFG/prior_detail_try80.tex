...
\begin{itemize}
\item \textbf{Global Head (Map-Level Context)}: The deepest bottleneck feature map is globally average-pooled to produce a single vector representing the macro-context. This pooled vector, fused with the height condition, is passed through an MLP to predict the non-spatial quantities: global mixture weights $\pi^{(g)}_{k,r,j}$, global offsets $\delta^{(g)}_{k,r,j}$, and global variances $\sigma^{(g)2}_{k,r,j}$.
...
$\hat y_{k,r} = \sum_j p_{k,r,j} \mu_{k,r,j}$};
...
\node[mix] (mu) at (17.0,-6.5) {Anchored means\\$\mu_{r,j}(x)=y^{\mathrm{prior}}_{k}(x)+\alpha_r(x)\Delta_{r,j}(x)$};
