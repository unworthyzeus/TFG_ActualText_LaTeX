...
Because the full \texttt{Try 80} run is still in training, this section
documents the frozen \texttt{Try 78}/\texttt{Try 79}
...
\(\log(1+y)\) space for the likelihood and converted back to native units for
metrics.
...
30~\mathrm{ns}, & k=\mathrm{DS},\ r=\mathrm{LoS},\\
40~\mathrm{ns}, & k=\mathrm{DS},\ r=\mathrm{NLoS},\\
9^\circ, & k=\mathrm{AS},\ r=\mathrm{LoS},\\
13^\circ, & k=\mathrm{AS},\ r=\mathrm{NLoS}.
\end{cases}
...
\(\sigma_{\min}=0.05\) and \(\sigma_{\max}=3.0\), preventing the NLL from
...
\texttt{Try 80} processes the map inputs through a shared multi-task convolutional encoder-decoder. Scalar UAV height is encoded into a condition vector $\textbf{c} \in \mathbb{R}^{128}$ and injected dynamically into the convolutional feature maps at multiple scales via \emph{Feature-wise Linear Modulation (FiLM)} layers. 
...
% Height MLP
\node[film] (h_mlp) at (-3.5, -4) {Height MLP \\ $\textbf{c} \in \mathbb{R}^{128}$};
...
\node[condbox] (mlp1) at (9.0,0) {Height MLP\\Linear $32\!\to\!128$\\SiLU\\Linear $128\!\to\!128$\\$\textbf{h}_{\mathrm{cond}}$};
...
\node[condbox] (fuse) at (18.0,0) {Condition fusion\\concat$\bigl[\textbf{h}_{\mathrm{cond}},\,\textbf{z}_{1:128}\bigr]$\\Linear $256\!\to\!128$\\SiLU\\Linear $128\!\to\!128$\\$\textbf{c}_{\mathrm{fused}}$};
...
\node[opbox] (filmproj) at (31.0,0.0) {Per-FiLM projection\\Linear $128\!\to\!2C$\\split into $\textbf{g},\textbf{b} \in \mathbb{R}^{C}$};
...
\caption{Detailed \texttt{Try 80} sinusoidal height-conditioning path.}
...
\node[global] (globin) at (0,3.5) {Global head input\\pooled bottleneck $+\ \textbf{c}_{\mathrm{fused}}$};
...
\node[local] (locin) at (0,0.5) {Local head input\\decoder map $\textbf{F}\in\mathbb{R}^{96\times H \times W}$};
...
\caption{Detailed prior-anchored GMM head used by \texttt{Try 80}.}
...
The implementation utilizes a robust composite loss to stabilize the GMM
training. For each task, let \(\Omega_k\) be the valid ground-pixel set,
let \(q_k(y\mid x)\) be the predicted mixture density, and let
\(\hat y_k(x)\) be the native-space point prediction used for RMSE. The total
objective is:
...
The numerical weights used in \texttt{try80\_joint\_big} are
\(\lambda_{\mathrm{NLL}}=1.0\), \(\lambda_{\mathrm{KL}}=0.25\),
\(\lambda_{\mathrm{mom}}=0.10\), \(\lambda_{\mathrm{anchor}}=0.05\),
\(\lambda_{\mathrm{guard}}=0.10\), \(\lambda_{\mathrm{out}}=0.02\),
\(\lambda_{\mathrm{RMSE}}=0.40\), and \(\lambda_{\mathrm{MAE}}=0.10\).
...
standard deviation. This prevents a low NLL solution from matching local
density while drifting in map-average level.
...
the design philosophy of \texttt{Try 80}: the DNN is allowed to improve the
prior, but it must pay a penalty for large or harmful deviations.

\paragraph{Outlier budget and deterministic losses.}
The last mixture component is treated as the broad/outlier component. If its
mean usage exceeds \(\tau=0.60\), a hinge penalty is applied:
\begin{equation}
...
\caption{Composite \texttt{Try 80} loss. The probabilistic terms train the
mixture density, while the anchor and guard terms enforce the prior-residual
interpretation.}
