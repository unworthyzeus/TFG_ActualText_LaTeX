...
\midrule
FSPL (LoS only)            & 3.8937 & ablation baseline (not used)\\
FSPL + radial residual     & 3.6505 & ablation baseline (not used)\\
Coherent two-ray (LoS)     & 1.7379 & building block $\to$ folded into Hybrid LoS\\
Hybrid LoS                 & 1.7379 & \textbf{frozen LoS prior} (used in Try 80)\\
Hybrid NLoS                & 3.3591 & \textbf{frozen NLoS prior} (used in Try 80)\\
\midrule
\textbf{Hybrid overall}    & \textbf{1.9072} & combined frozen prior (Try 80 input)\\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Why do Coherent two-ray (LoS) and Hybrid LoS show the same RMSE?}
The two rows report the same value (1.7379 dB) on the \emph{evaluation split},
...
\addplot[fill=blue!50] coordinates {
  ...
  (Hybrid LoS,1.7379)
  (Hybrid NLoS,3.3591)
  (Hybrid overall,1.9072)
};
\end{axis}
\end{tikzpicture}
\caption{Try 78 pixel-weighted RMSE values. The coherent two-ray model
reduces LoS RMSE from 3.89 dB (FSPL) to 1.74 dB, a gain of 2.2 dB.}
...
\item fully global fallback: (metric, global, all\_los, all-ant), where
      \texttt{all\_los} denotes all visibility states rather than LoS-only data.
\end{enumerate}
