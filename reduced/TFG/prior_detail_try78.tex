...
The dominant pattern is $\rho \approx 0.46$ to $0.56$ with $\phi \approx 0$
for heights 10 to 100 m, meaning a moderate-amplitude ground reflection with
a near-zero fitted extra phase offset. Biases are near-zero ($< 0.05$ dB) for these bins. At very high
altitudes, the fitted $\rho$ and bias increase because training data is sparser;
those bins are stabilized by Gaussian smoothing.
...
range $0.46$ to $0.56$ for 10 to 200 m, then rise at the sparsely sampled
...
for urban air-to-ground path loss. UAV channel models often report fewer
obstructions and lower shadowing variance at larger elevation angles
\cite{khawaja_survey,tr38901}, but the particular constants below are not used
as a standalone monotonic law. They define one raw envelope that is later
combined with COST-231 through a maximum and corrected by the train-only
regime-wise NLoS calibration.
