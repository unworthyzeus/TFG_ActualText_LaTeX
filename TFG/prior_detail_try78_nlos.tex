...
The full raw prior combines LoS and NLoS estimates using the binary LoS mask $m_{\mathrm{LoS}}(x) \in \{0,1\}$:
\begin{equation}
\mathrm{PL}^{(0)}(x) = m_{\mathrm{LoS}}(x)\,\mathrm{PL}_{\mathrm{LoS,blend}}(x) + \left(1-m_{\mathrm{LoS}}(x)\right)\mathrm{PL}_{\mathrm{NLoS,raw}}(x)
\label{eq:pl0}
\end{equation}
Since $m_{\mathrm{LoS}}(x)$ is binary, this is a hard switch between the LoS and NLoS expressions. Eq.~\eqref{eq:los_blend} defines a legacy auxiliary soft blend inside the LoS branch:
\[
\mathrm{PL}_{\mathrm{LoS,blend}}(x) = 0.7\,\mathrm{PL}_{\mathrm{LoS,path}}(x) + 0.3\min\!\left(\mathrm{PL}_{\mathrm{LoS,path}}(x),\,\mathrm{PL}_{\mathrm{A2G,LoS}}(x)\right).
\]
Here $\mathrm{PL}_{\mathrm{LoS,path}}(x)$ is the legacy LoS path term from the old formula prior, not the calibrated coherent two-ray map. In the final frozen \texttt{Try 80} path, LoS pixels are taken directly from the calibrated two-ray prior, so this mixed expression is retained only for historical completeness.
...
The stored calibration for this legacy mixed-prior map still contains both LoS and NLoS regime coefficients. However, in the final frozen \texttt{Try 80} path-loss assembly only the NLoS pixels from this calibrated map are used, because LoS pixels are replaced by the separate calibrated coherent two-ray prior.
