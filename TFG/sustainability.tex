...
Tries~76 and 77 (the 12-expert distribution-first training) also used the
same four-GPU node, adding roughly \SI{18}{h} of GPU time and about
\SI{27}{kWh}. The non-deep-learning priors of Tries~78 and~79 do not need the
cluster: the calibration code runs on a single GPU or CPU through the
DirectML backend, takes a few hours end-to-end on a consumer laptop, and is
mentioned here only for completeness since the main carbon cost comes from
the deep-learning stages. The complete exploratory project exceeded
...
performance without field measurements. The final Try 80 results are reported
