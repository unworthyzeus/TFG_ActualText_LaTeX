...
justify the final route, the detailed frozen prior formulas, and
\textsc{HARP-Net CKM}. The longer derivation files remain in the
repository as source-level mathematical notes; the compiled text below keeps
the same formulas in a shorter paper-style form.

\section{Dataset and evaluation contract}

Each sample is a complete \(513\times513\) CKM with
\SI{1}{m}\(\times\)\SI{1}{m} pixels. The transmitter is always
horizontally centred in the map, but its antenna height \(h_{\mathrm{tx}}\)
varies by sample and is one of the main physical conditioning variables. The
CKM files call this elevated node a UAV, but the same \SI{7.125}{GHz} contract
also represents other elevated radio access nodes within the sampled height
range. Every
...
\item The final channel attenuation and spread priors showed that large parts of the
...
structure and \textsc{HARP-Net CKM} learns bounded local corrections.
\end{enumerate}
...
\node[model] (net) at (10.2,0) {Shared U-Net\\FiLM height conditioning\\residual heads};
...
\end{tikzpicture}}
\caption{\textsc{HARP-Net CKM} pipeline. Frozen height aware priors are
computed first and then used as input channels for the final residual model.}
\label{fig:pipeline}
\end{figure}
...
The path loss prior has two branches.
...
\label{eq:nlos_raw}
\end{equation}
The final NLoS estimate is not this raw formula. It is a per regime linear
calibration over morphology and height features:
\begin{equation}
\mathrm{PL}_{\mathrm{NLoS}}
=\operatorname{clip}(\mathbf{x}_{78}^{\top}\mathbf{w}_{78,r},20,185),
...
shadowing proxy and \(\theta_{\mathrm{norm}}\) is normalized elevation. The
interaction terms let density and obstruction support matter differently at
different ranges and elevations. The coefficient vector
...
The spread priors are fitted in
\(\log(1+y)\) space:
\begin{equation}
z=\log(1+y),
\qquad
...
\section{\textsc{HARP-Net CKM} residual head}
\label{sec:try80_detail}

The final \textsc{HARP-Net CKM} model predicts residual distributions around
the frozen priors. For target
...
map exported for metrics is the local-weighted model-space mean, followed by
...
\end{equation}
For path loss, \(\hat y_t=\hat z_t\); for delay and angular spread,
\(\hat y_t=\exp(\hat z_t)-1\).
...
\node[head] (pl) at (6.6,1.1) {PL mixture branch\\\(\pi,\mu,\sigma\)};
\node[head] (ds) at (6.6,0) {DS mixture branch\\\(\pi,\mu,\sigma\)};
\node[head] (as) at (6.6,-1.1) {AS mixture branch\\\(\pi,\mu,\sigma\)};
...
\end{tikzpicture}}
\caption{\textsc{HARP-Net CKM} residual head structure. The shared network predicts
