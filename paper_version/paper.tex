...
maps used for RMSE reporting. On held out cities it reaches \SI{1.6950}{dB}
channel attenuation RMSE,
\SI{26.6915}{ns} delay spread RMSE, and \(11.4197^\circ\) angular spread RMSE,
...
\SI{1.6950}{dB} because it emphasizes hard tails. Less quantized labels would
...
\([20,185]\) dB range. The notation is local: \(\rho(h_{\mathrm{tx}})\) is the
...
=\operatorname{clip}(\mathbf{x}_{\mathrm{NLoS}}^{\top}\mathbf{w}_{\mathrm{NLoS},r},20,185).
...
Angular spread & 11.4197 & 13.7416 & -2.3220 & 4.8597 & deg \\
\bottomrule
\end{tabular}
\end{table*}
...
MapCorr values rise by about \(36\%\) and \(58\%\) relative to the frozen priors.
Thus the residual model is doing visible map-structure correction even when the
...
reduces these to \SI{1.6950}{dB} and \SI{0.9773}{dB}. This is the most
...
MAE; the final model reaches \(11.4197^\circ\) RMSE and \(4.8597^\circ\) MAE.
...
The delay spread prior reaches \SI{28.1211}{ns} RMSE and \SI{7.3033}{ns} MAE;
the final model reaches \SI{26.6915}{ns} RMSE and \SI{7.7878}{ns} MAE. This
...
\midrule
Atten. & LoS & 1.5177 & 1.7519 & -0.2342 \\
Atten. & NLoS & 3.1502 & 3.5143 & -0.3641 \\
DS & LoS & 25.6189 & 27.5119 & -1.8930 \\
DS & NLoS & 29.2455 & 29.6157 & -0.3702 \\
AS & LoS & 12.3914 & 15.2818 & -2.8904 \\
AS & NLoS & 8.4556 & 8.6614 & -0.2058 \\
\bottomrule
\end{tabular}
\end{table}
...
Test & 2590 & 1.6950 & 26.6915 & 11.4197 \\
\bottomrule
\end{tabular}
\end{table}
...
on the full training plus held out pool. The final paper therefore keeps the
prior only audits above and the held out test results in
Tables~\ref{tab:main-results}--\ref{tab:val-test}; the longer per group tables
belong in the thesis and evaluation artifacts.
...
\textbf{\SI{1.6950}{dB} attenuation; \SI{26.6915}{ns} DS; \(11.4197^\circ\) AS} &
...
\textbf{Final model} &
\textbf{\SI{26.6915}{ns} DS RMSE and \(11.4197^\circ\) AS RMSE.} &
...
continuous channel attenuation, DS, and AS values; the \SI{0.9773}{dB} final
...
Under strict city holdout, the final model reaches \SI{1.6950}{dB},
\SI{26.6915}{ns}, and \(11.4197^\circ\), respectively. Its main contribution is
...
\SI{0.2433}{dB}, \SI{1.4296}{ns}, and \(2.3220^\circ\) RMSE on the held out
test set. It also improves map correlation from 0.947710 to 0.961054 for
attenuation, from 0.306026 to 0.414823 for delay spread, and from 0.377700 to
0.595179 for angular spread. For the two spread targets this is a relative
MapCorr gain of about \(36\%\) and \(58\%\), respectively, both above the
