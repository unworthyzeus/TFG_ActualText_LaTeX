...
The main reported model is \textsc{HARP-Net CKM},
evaluated on 2590 samples from 14 unseen test cities. All model-versus-prior
rows use the same held out split and the same valid ground receiver mask.
...
Ground mask reset & Building pixels are not receivers and must be excluded. & Later metrics use the valid ground receiver mask. \\
...
\textsc{HARP-Net CKM} & A bounded neural residual improves already strong priors. & Test: \SI{1.6519}{dB} PL, \SI{26.5570}{ns} DS, \(11.3854^\circ\) AS. \\
\bottomrule
\end{tabularx}
\end{table}

The final channel attenuation prior is the most important baseline. Its LoS branch is not
FSPL alone: the enhanced two ray calibration reduces LoS RMSE from about
\SI{3.95}{dB} to \SI{1.75}{dB}. The full hybrid prior reaches
...
All-city group range & 1269--2156 & -0.2363 to -0.3602 & -0.5428 to -3.1170 & -1.2874 to -2.8587 \\
\bottomrule
\end{tabular}
\end{table}
...
PMNet / ICASSP \cite{pmnet_icassp2023,icassp2023challenge} & approx.\ \SI{1.38}{dB} converted score & Final PL is close but higher under a different benchmark contract; scale context only. \\
...
AIRMap \cite{airmap2025} & Path-gain error \(<\SI{4}{dB}\) against ray tracing & Same digital-twin motivation, different target and fixed \(200\times200\) input grid. \\
