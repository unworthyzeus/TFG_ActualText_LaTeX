...
used by Try80 are the smoothed $\rho$, $\phi$, and $b$ curves themselves, not
...
feature for the calibrated Try78 NLoS branch.
...
the scalar formula input to the calibrated Try78 NLoS branch. It combines the
...
For the Try78 path-loss calibration, these two quantities are converted into
three coarse city types. This is a separate regime label from the later
six-class topology taxonomy used by the Try79 spread priors and by final
reporting:
...
label and the three UAV-height bins below, this yields the 18 stored Try78
...
Try78 city type and LoS/NLoS label. It does not use the broader cross-topology
fallback hierarchy of the Try79 spread-prior calibration.
