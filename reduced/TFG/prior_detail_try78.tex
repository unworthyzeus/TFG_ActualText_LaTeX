...
shape, while per regime fitting adapts the coefficients to the CKM simulator
and to the dataset's stored masks.

\emph{Implementation note.} The first coefficient in
\eqref{eq:pl_nlos_dense_highrise_mid_coefficients} multiplies
\texttt{prior\_sq}, the quadratic raw NLoS prior feature \(P_0(x)^2\). The
released code keeps this term because the stored calibration vector is the
result of the final full calibration run, which was expensive to repeat.
However, in the final code path, deleting this feature or setting only this
coefficient to zero produces no meaningful change in the resulting path loss
maps. Therefore the quadratic term has little practical influence compared with
the linear raw prior, morphology, elevation and bias terms. In a later
calibration iteration this feature could, and probably should, be removed; it
is kept here mainly as an artifact of the calibration process.
