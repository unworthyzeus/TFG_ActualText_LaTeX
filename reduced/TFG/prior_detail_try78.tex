...
\emph{Implementation note.} The coefficient attached to \texttt{prior\_sq} in
\eqref{eq:pl_nlos_dense_highrise_mid_coefficients} multiplies the
\(\mathrm{PL}^{(0)}(x)^2\) basis term, a quadratic copy of the raw path loss
prior. It is kept because the stored final calibration vector was expensive to
rerun. Follow-up tests showed that removing the feature or zeroing this
coefficient does not meaningfully change the maps; the linear raw prior,
morphology, elevation and bias terms carry the practical effect. A later
calibration should drop it.
