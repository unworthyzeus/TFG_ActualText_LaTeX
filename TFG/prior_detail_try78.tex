...
pixels. Eq.~\eqref{eq:los_blend} defines a legacy auxiliary soft blend inside
the LoS branch:
...
Here $\mathrm{PL}_{\mathrm{LoS,path}}(x)$ denotes the \emph{legacy} LoS path
term used in older formula-based variants: FSPL at short range and the
classical asymptotic two-ray crossover law at longer range. It is not the
calibrated coherent two-ray map of Eq.~\eqref{eq:pl_los}.

This auxiliary blend was used in older \texttt{Try 78} variants to keep the
LoS formula below an A2G envelope. In the final frozen \texttt{Try 80} path,
LoS pixels come directly from the calibrated coherent two-ray prior, so
Eq.~\eqref{eq:pl0} and Eq.~\eqref{eq:los_blend} are retained only for
