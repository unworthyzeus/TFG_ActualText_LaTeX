...
\begin{itemize}
  ...
  \item A calibrated non-deep-learning path-loss prior (Try~78) based on an
  ...
  \item A calibrated non-deep-learning spread prior (Try~79) combining
  ...
  \item A prior-anchored joint neural residual model (Try~80) that shares a
        single backbone across the three targets, conditioned on the frozen
        Try~78 and Try~79 priors.
  \item A distribution-first diagnosis of the previous PMHHNet family
        (Tries~66--75) that identifies NLoS mode collapse and motivates the
        Try~76 GMM architecture. The distribution analysis is released as
...
axes along which Try~80 is positioned. The methodology chapter describes the
CKM dataset, the ground-mask protocol, the attempt-evolution table, the shared
PMHHNet architecture with sinusoidal height conditioning, and the four
prior-detail subsections for Tries~78, 79 and 80. The results chapter reports
the quantitative outcomes: the Try~78 LoS waterfall, per-expert and per-height
breakdowns, Try~79 spread results, and the final Try~80 test evaluation on
unseen cities. The sustainability and conclusions chapters close the thesis,
followed by the appendices with the representative validation panels and the Try~80
