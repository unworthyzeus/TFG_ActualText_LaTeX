...
\textbf{RQ1: dense prediction under city holdout.} Dense CKM prediction is
feasible under the strict protocol used in this thesis. The final model
predicts PL, DS, and AS jointly on unseen cities and clears the original
...
within the fixed CKM contract: one carrier frequency, one centred UAV, valid
ground receivers only, and city-level train/validation/test separation. Within
that contract, the answer is yes.

\textbf{RQ2: prior versus neural prediction.} The answer is not ``physics or
neural prediction'', but both in the right order. The calibrated priors put
together the most predictable pieces of the problem: two-ray LoS path loss,
...
\textbf{RQ3: continuous UAV height.} Continuous UAV height can be handled when
it is represented explicitly. In the final model, height enters twice: through
height-aware calibrated priors and through a sinusoidal FiLM conditioning path
inside the neural residual model. This is important because low-altitude cases
are not just lower versions of high-altitude cases; they contain more blocked
links, stronger transition zones, and harder spread tails. The height-binned
results show that high-altitude samples are easier even though they are less
represented in the training data, which confirms that the effect is physical
