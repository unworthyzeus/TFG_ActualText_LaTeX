...
nlos_prior = _compute_path_nlos_map(topology, los_mask, h_tx,
                                    self._path_nlos_cal)
path_prior = np.where(los_mask > 0.5, los_prior, nlos_prior)

delay_prior = _compute_spread_metric_prior("delay_spread", ct6, ant,
                                           los_mask, shared, self._spread_cal)
angular_prior = _compute_spread_metric_prior("angular_spread", ct6, ant,
                                             los_mask, shared, self._spread_cal)
\end{verbatim}
}
...
The final benchmark was repeated on ten exported HDF5 samples using the full
generator computation after the checkpoint had been loaded once. The timed path
therefore includes mask/prior preparation and neural prediction, but not the
one-time checkpoint load. The CPU fallback was measured on an AMD
Ryzen~5~5600H with 6 physical cores, 12 logical threads, and 6 PyTorch compute
threads. It took \SIrange{5.60}{6.50}{s} per map, with a mean of
\SI{5.95}{s}. On the RTX~3050~Ti Laptop GPU, which has 4\,GiB of VRAM and
2{,}560 CUDA cores, the fp32 batch-1 pipeline took \SI{1.40}{s} per map.
CUDA mixed precision (fp16 autocast) with batch size 1 reduced this to
\SI{1.14}{s} per map.

The batch-size behaviour was limited by memory rather than arithmetic. Mixed
precision (fp16 autocast) with batch size 2 used about 2.65\,GiB of peak
allocated GPU memory and took \SI{1.17}{s} per map, so it did not improve
throughput on this laptop GPU. Mixed precision (fp16 autocast) with batch
size 4 reported about 5.11\,GiB of peak allocated GPU memory on a 4\,GiB device
and slowed to \SI{6.36}{s} per map. This indicates Windows/CUDA
over-subscribing VRAM and spilling part of the working set into shared system
RAM. The RTX~2080~Ti used for the
ray-tracing generation has 11\,GiB of VRAM and 4{,}352 CUDA cores, so a
same-class desktop GPU would be expected to fit larger batches and expose more
parallelism; that hardware was not available for a direct surrogate benchmark.
