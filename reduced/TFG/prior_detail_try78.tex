...
then clipped to the \([20,180]\) dB physical path loss range used by the final
...
\subsubsection{\texorpdfstring{\emph{Raw NLoS propagation prior}}{Raw NLoS propagation prior}}
...
the uppercase design matrix \(\textbf{X}_r\). For compact table notation, let
\(\ell_d(x)=\log(1+d_{2D}(x))\). Shared support maps such as \(d_{2D}\),
\(\theta_{\mathrm{norm}}\), \(\delta_k\), \(h_k\), and \(n_k\) are defined once
in Sec.~\ref{sec:support_geometry_morphology}; Table~\ref{tab:pl_nlos_term_inventory}
only states how they enter the NLoS calibration.
