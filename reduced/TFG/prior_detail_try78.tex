...
Each term is now described in detail. The ground mask \(g\), building occupancy
mask \(m_{\mathrm{B}}\), LoS/NLoS masks, \(d_{2D}\), \(\theta_{\mathrm{norm}}\),
\(\delta_k\), \(h_k\), and \(n_k\) are the shared support maps defined in
...
definitions as the rest of the chapter.
...
The density terms \(\delta_{15},\delta_{41}\) measure local building coverage.
...
given in \eqref{eq:density_support} to \eqref{eq:n_support} and
