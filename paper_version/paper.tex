...
This paper reports a dense UAV CKM surrogate under a
strict city-holdout contract. The answer to the first research question is
positive within that contract, but the path to that answer matters. The earlier
distribution-aware GMM heads without frozen priors were not the best final
predictors; they were worse than the final prior-anchored system. Their value
was diagnostic: they showed that the earlier failures were not caused by
path loss, delay spread, and angular spread being impossible dense targets, but
by using output heads and losses that collapsed multi-modal or spike-plus-tail
distributions. The final shared model then closes RQ1 by predicting all three
maps jointly in one generator, reaching \SI{1.6519}{dB},
\SI{26.5570}{ns}, and \(11.3854^\circ\) on unseen cities.

The answer to the second question is that the useful split is not ``physics or
neural'', but calibrated physics/statistics first and bounded neural residuals
second. Frozen priors explain most radial, visibility, height, and spread-scale
structure; the final model improves those priors by \SI{0.2865}{dB},
\SI{1.5453}{ns}, and \(2.3562^\circ\) RMSE on the held-out test set. The
answer to the height question is also positive but conditional: continuous UAV
height can be handled by height-aware priors plus sinusoidal embeddings and
FiLM modulation, so one model can cover the varying-height range without
per-altitude retraining, but low-altitude and NLoS-heavy cases remain the
regimes that require explicit reporting.

The main practical conclusion is therefore narrow and useful. For this fixed
ray-traced CKM simulation contract, a prior-anchored residual GMM-head model is
