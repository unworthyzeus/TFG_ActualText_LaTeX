...
pass plus the default uncompressed \texttt{.npz} numerical archive export.
Optional mask PNGs and rendered map PNGs are not part of this timing. The
...
component, with the fast \texttt{.npz} export small enough to be absorbed into
the rounded prediction/export timing rather than reported as a separate
bottleneck.
...
generation path, including masks, priors, predictions, and the default
numerical \texttt{.npz} export.
