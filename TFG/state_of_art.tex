...
than CKM's effective 65 to 130~dB range, intrinsically compressing RMSE values.
...
under a random 90/10 split on \(256\times256\) maps.  This supports the use of
attention for long-range urban context, but the score is not inserted in
Table~\ref{tab:icassp2023corrected} because it is not measured on
RadioMap3DSeer and cannot be converted with that dataset's \SI{36}{dB}
...
Gao et al.\ \cite{gao2026} & No & No & No & No \\
...
The key lesson for the final model is not that the numerical RMSE should match
...
conditions. This supports the decision to report the final model under a strict
...
ability to produce dense radio maps fast enough for planning loops. The final model is
...
that appeared repeatedly in the internal experiments: the physically important region
...
under-test multi-transmitter or shifted building-density regimes. The final model is
...
The comparison to the PMHHNet reference should therefore be read as diagnostic rather than
competitive. Its \SI{9.41}{dB} path-loss result is not claimed to beat
indoor methods; it is used to argue that the earlier CKM direct-regression
family had reached the same type of NLoS-dominated wall. The final priors and residual GMM-head model were
...
Across the benchmarks reviewed here, the recurring $9$ to $10\,\mathrm{dB}$
error band in harder NLoS-heavy settings is unlikely to be only a dataset
scaling artefact; it reflects the fundamental difficulty of inferring volumetric
wave propagation from a 2D building height map.  Closing this gap requires
either richer inputs, stronger physical constraints, or stochastic modelling of
the aleatoric uncertainty.  Table~\ref{tab:emerging} summarises the techniques
with the highest estimated impact for CKM-style evaluation.

\begin{table}[ht]
\centering
\caption[Emerging techniques for future CKM work.]%
{Emerging techniques that could be applied on top of the current
...
2 & PINN PDE residual loss & ReVeal \cite{reveal2025} & High \\
3 & Dual LoS/NLoS residual heads & PMHHNet diagnostics & Low \\
4 & Target-domain adaptation from sparse samples & RadioPiT \cite{radiopit2025} & Low to medium \\
...
Items~1 to 3 already exist in partial form inside the PMHHNet diagnostics. Items~4 to 6
...
The reviewed literature does not point to a single method that can simply be
copied for the CKM task. Instead, each family contributes one useful ingredient
and leaves one gap that shaped the thesis design. Table~\ref{tab:soa_gap_synthesis}
summarises this mapping.

\begin{table}[ht]
\centering
\caption[Literature families and thesis design response.]%
{Synthesis of literature families and the corresponding thesis design
response. The table is not a ranking; it identifies which limitation each
method family exposed for the CKM setting.}
\label{tab:soa_gap_synthesis}
\scriptsize
...
\toprule
\textbf{Literature family} & \textbf{Contribution} & \textbf{Remaining gap for this thesis} & \textbf{Thesis response} \\
\midrule
U-Net, pix2pix, and cGAN radio-map predictors &
Dense image-to-image mapping. &
Visual plausibility can hide physical RMSE and NLoS tail errors. &
Use ground-only physical metrics, LoS/NLoS reporting, and reject visual realism
as the primary objective. \\

PMNet and PMHHNet-style backbones &
Good spatial inductive bias and multi-scale context. &
Mostly single-target or benchmark-specific; does not solve target-distribution
mismatch. &
Use distribution-first heads, GMM marginals, and regime diagnostics before
choosing the final residual model. \\

Classical propagation and two-ray models &
Strong LoS path-loss structure. &
Weak for blocked urban NLoS and for spread targets. &
Use the final LoS prior plus a calibrated NLoS morphology model. \\

Empirical spread models &
Log-domain and regime-dependent spread statistics. &
Usually not dense pixel maps with building morphology. &
Use log-domain ridge priors with topology, height, and LoS/NLoS regimes. \\

Heteroscedastic, GMM, and uncertainty heads &
Multi-modal or heavy-tailed targets can be represented directly. &
Often not anchored to physical priors. &
Use a prior-anchored residual GMM-head model. \\

AIRMap, PathFinder, and adaptive methods &
Fast inference, calibration, or domain-shift awareness. &
Different task assumptions and benchmark contracts. &
Use an explicit fair-comparison protocol and strict city-holdout interpretation. \\
\bottomrule
\end{tabularx}
\end{table}
\normalsize

This synthesis is the main reason the final system is hybrid. A pure neural
model would reuse the dense prediction machinery but ignore the strong LoS
structure. A pure propagation model would explain the radial path-loss field
but miss blocked streets and spread tails. A pure uncertainty head would model
the heavy-tailed targets but would still have to rediscover the easy physics.
The final model combines the pieces that survived those comparisons: calibrated priors,
explicit regimes, distribution-aware residuals, and a city-holdout evaluation
contract.

% Section separator
\section{Position of the Final Model in the Literature}
\label{sec:soa_position}
% Section separator

\subsection{What is novel about the final model}

The final model occupies a distinct and innovative position in the radio-map literature
...
\begin{itemize}
  \item \textbf{Distribution-first dense prediction.}  Prior MDN and GMM
        ...
        The final residual GMM-head model transfers that principle to dense radio maps: the model first
  ...
  \item \textbf{Three simultaneous dense targets.} The final model jointly predicts
  ...
  \item \textbf{Frozen physics anchors.}  Two independently calibrated
        physics-based priors, the enhanced two-ray model for path
        loss and the log-space ridge regression for both spreads, are
        ...
        requiring the neural component to re-learn it.  The anchors are also a
        strong baseline in their own right: they encode train-only calibration
        over visibility, morphology, density, building height, and UAV-height
        regimes before any residual network is applied.
...
\subsection{What the final model does not claim}

The final model does not claim to replace full ray-tracing in arbitrary scenarios. The
...
systematic correction of the protocol axes listed in that section.

The final model is also not presented as a universal radio-map foundation model. It is a
surrogate for a specific CKM dataset contract: fixed output definitions, a
known simulator family, a \(513\times513\) spatial grid, explicit masks, and
continuous UAV height within the dataset range. Generalising beyond this scope
would require additional conditioning variables such as carrier frequency,
bandwidth, antenna pattern, material information, and field measurements.
...
carefully audited CKM predictor under a known dataset contract. The comparison
is therefore intentionally narrower and more honest. It asks whether the model
can reproduce unseen-city CKM maps under the same physical-unit evaluation used
throughout the project, not whether it can replace every ray tracer or every
radio-map benchmark.

\subsection{Summary of position}

Table~\ref{tab:position} provides a compact summary of how the final model compares to
...
AIRMap \cite{airmap2025} & path-gain $<4$\,dB vs.\ ray tracing; field calibration reported as median percentage error & Partial & Different protocol and path-gain target. \\
...
PMHHNet reference (this thesis) & 9.41\,dB & Yes & Internal direct-regression baseline. \\
ML-calibrated path-loss prior & 1.92\,dB overall; 1.75\,dB LoS & Yes & Frozen path-loss prior used by the final pipeline. \\
