...
path-loss scale. More recent work on noisy indoor radio-map prediction
\cite{jaensch2026noisyenv} reports measured-data RMSE between
\SI{1.3}{dB} and \SI{2.1}{dB}, depending on whether complete or noisy
environment information is available. These papers are useful path-loss scale
references, but they do not report dense delay-spread or angular-spread maps.
...
The closest external quantitative references are therefore scalar channel
characteristic predictors. Mi et al.\ \cite{mi2024pointcloud} use measured
\SI{60}{GHz} indoor data, fused point clouds, RGB information, and segmentation
masks to predict path loss, RMS delay spread, angular spread, and Rician
\(K\)-factor, reporting mean RMSE values of \SI{1.56}{dB}, \SI{0.90}{ns},
\(7.39^\circ\), and \SI{2.67}{dB}, respectively. Huang et al.\
\cite{huang2025a2gtransformer} predict UAV A2G channel characteristics with a
Transformer model; on merged datasets they report \(0.172\,\mathrm{dBm}\)
received-power RMSE, \SI{4.577}{ns} RMS delay-spread RMSE, and angular-spread
component RMSE values from \(0.074^\circ\) to \(1.549^\circ\). These numbers
are not dense-map leaderboards, but they are the fairest external anchors for
asking whether the final spread errors are in the right order of magnitude.
...
medium-priority future work. The final model is not multi-transmitter, but the
same concern motivates its city-holdout split: a CKM surrogate is useful only if
it works on new urban layouts, not only on shuffled samples from the same city.
...
Noisy-environment CNN \cite{jaensch2026noisyenv} & 1.3--2.1\,dB measured indoor path-loss RMSE & No & Useful measured-data path-loss scale; sparse observations and indoor setting. \\
Mi et al.\ point-cloud LSP predictor \cite{mi2024pointcloud} & 1.56\,dB PL; 0.90\,ns DS; \(7.39^\circ\) AS & No & Closest measured multi-parameter spread reference, but scalar indoor snapshots. \\
Huang et al.\ A2G Transformer \cite{huang2025a2gtransformer} & 4.577\,ns DS; \(0.074^\circ\)--\(1.549^\circ\) angular components & No & Closest UAV spread predictor, but channel-characteristic samples rather than dense maps. \\
